...
// defaults set.
func ExampleFile() {
	// This will load a over_config file from ./over_config.yaml
	loader := over_config.File()
	if _, err := loader.Complete(); err != nil {
		fmt.Println("failed to load over_config")
		os.Exit(1)
	}
}
...
func ExampleFile_atPath() {
	loader := over_config.File().AtPath("/var/run/over_controller-runtime/over_config.yaml")
	if _, err := loader.Complete(); err != nil {
		fmt.Println("failed to load over_config")
		os.Exit(1)
	}
