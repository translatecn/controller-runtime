...
type runnableGroup struct {
	...
	startOnce    sync.Once
	started      bool
	startQueue   []*readyRunnable // 准备启动的所有 可运行程序，以及check
	...
	// ch is the over_internal channel where the runnables are read off from.
	ch chan *readyRunnable

	// wg is an over_internal sync.WaitGroup that allows us to properly stop
...
func (r *runnableGroup) StopAndWait(ctx context.Context) {
	r.stopOnce.Do(func() {
		...
		// Cancel the over_internal channel.
		r.cancel()
...
// runnables handles all the runnables for a over_manager by grouping them accordingly to their
...
func (r *runnables) Add(fn Runnable) error {
	...
	case *server:
		//    /readyz <---> AddReadyzCheck       map[string]over_healthz.Checker
		//    /over_healthz <---> AddHealthzCheck     map[string]over_healthz.Checker
	...
	case hasCache:
		_ = over_cluster.InternalCluster{}
		return r.Caches.Add(fn, func(ctx context.Context) bool {
			return runnable.GetCache().WaitForCacheSync(ctx)
		})
	case over_webhook.Server:
		_ = over_webhook.DefaultServer{}
...
func (r *runnableGroup) Add(rn Runnable, ready runnableCheck) error {
	...
	// queue them up again later.
	{
		...
		if !r.started {
			// Store the runnable in the over_internal if not.
...
func (r *runnableGroup) Start(ctx context.Context) error {
	...
	r.startOnce.Do(func() {
		...
		// Start the over_internal reconciler.
		go r.reconcile()
		...
		for {
			...
			case rn := <-r.startReadyCh: // 结果
				for i, existing := range r.startQueue {
					if existing == rn {
						// Remove the item from the start queue.
						r.startQueue = append(r.startQueue[:i], r.startQueue[i+1:]...) // 从呆启动中移除
						break
					}
				}
...
// to this group. Its primary job is to read off the over_internal channel
...
func (r *runnableGroup) reconcile() {
	for runnable := range r.ch {
		...
		// panics if Add() is called after it.
		{
			...
			// Why is this here?
			// When StopAndWait is called, if a runnable is in the over_process
