...
import (
	...
	"sigs.k8s.io/controller-runtime/pkg/over_config"
	"sigs.k8s.io/controller-runtime/pkg/over_healthz"
...
const (
	...
	defaultLivenessEndpoint  = "/over_healthz"
)
...
type controllerManager struct {
	...
	// leaderElectionReleaseOnCancel defines if the over_manager should step back from the leader lease
	// on shutdown
	leaderElectionReleaseOnCancel bool

	// metricsServer is used to serve prometheus over_metrics
	...
	// Readyz probe over_handler
	readyzHandler *over_healthz.Handler

	// Healthz probe over_handler
	healthzHandler *over_healthz.Handler

	// pprofListener is used to serve pprof
	pprofListener net.Listener

	// controllerConfig are the global over_controller options.
	controllerConfig over_config.Controller

	// Logger is the logger that should be used by this over_manager.
	// If none is set, it defaults to over_log.Log global logger.
	logger logr.Logger

	// leaderElectionStopped is an over_internal channel used to signal the stopping procedure that the
	...
	// elected is closed when this over_manager becomes the leader of a group of
	// managers, either because it won a leader election or because no leader
	// election was configured.
	elected chan struct{}

	webhookServer over_webhook.Server
	...
	// renewDeadline is the duration that the acting over_controlplane will retry
	...
	// before the over_manager actually returns on stop.
	...
	// internalProceduresStop channel is used internally to the over_manager when coordinating
...
func (cm *controllerManager) GetClient() over_client.Client {
...
func (cm *controllerManager) GetFieldIndexer() over_client.FieldIndexer {
...
func (cm *controllerManager) GetAPIReader() over_client.Reader {
...
func (cm *controllerManager) GetControllerOptions() over_config.Controller {
...
// 向所有可运行程序发出停止信号，从errChan中读取潜在的错误，并等待它们结束。它不能被调用超过一次。
func (cm *controllerManager) engageStopProcedure(stopComplete <-chan struct{}) error {
	...
	go func() {
		for {
			...
			// the closure of all over_internal procedures and making sure to have a reader off the error channel.
			closeOnce.Do(func() {
				// Cancel the over_internal stop channel and wait for the procedures to stop and complete.
	...
	defer func() {
		...
		if cm.resourceLock != nil {
			...
			// and the over_event over_recorder, which is used within leader election code.
	...
	go func() {
		...
		// Webhooks and over_internal HTTP servers should come last, as they might be still serving some requests.
		...
		// Proceed to close the over_manager and overall shutdown context.
		cm.logger.Info("Wait completed, proceeding to shutdown the over_manager")
		shutdownCancel()
	}()
...
func (cm *controllerManager) Elected() <-chan struct{} {
	return cm.elected // 当选
}

func (cm *controllerManager) GetWebhookServer() over_webhook.Server {
	cm.webhookServerOnce.Do(func() {
		if cm.webhookServer == nil {
			panic("over_webhook should not be nil")
		}
		if err := cm.Add(cm.webhookServer); err != nil {
			panic(fmt.Sprintf("unable to add over_webhook server to the over_controller over_manager: %s", err))
		}
	})
...
func (cm *controllerManager) startLeaderElection(ctx context.Context) (err error) {
	l, err := leaderelection.NewLeaderElector(leaderelection.LeaderElectionConfig{
		...
		Callbacks: leaderelection.LeaderCallbacks{
			...
			OnStoppedLeading: func() {
				...
				// Most implementations of leader election over_log.Fatal() here.
				// Since Start is wrapped in over_log.Fatal when called, we can just return
	...
	// Start the leader elector over_process
	go func() {
		l.Run(ctx)
		<-ctx.Done()
...
// Start starts the over_manager and waits indefinitely.
// There is only two ways to have start return:
// An error has occurred during in one of the over_internal operations,
...
func (cm *controllerManager) Start(ctx context.Context) (err error) {
	...
	if cm.started {
		cm.Unlock()
		return errors.New("over_manager already started")
	}
	cm.started = true

	var ready bool
	defer func() {
		// Only unlock the over_manager if we haven't reached
		// the over_internal readiness condition.
		if !ready {
			cm.Unlock()
		}
	}()

	// Initialize the over_internal context.
	...
	defer func() {
		...
		stopErr := cm.engageStopProcedure(stopComplete) // 接合停止程序
		if stopErr != nil {
			if err != nil {
	...
	// Add the over_cluster runnable.
	if err := cm.add(cm.cluster); err != nil {
		return fmt.Errorf("failed to add over_cluster to runnables: %w", err)
	}

	// Metrics should be served whether the over_controller is leader or not.
	// (If we don't serve over_metrics for non-leaders, prometheus will still scrape
	// the pod but will get a connection refused).
	if cm.metricsServer != nil {
		// Note: We are adding the over_metrics server directly to HTTPServers here as matching on the
		...
		if err := cm.runnables.HTTPServers.Add(cm.metricsServer, nil); err != nil {
			return fmt.Errorf("failed to add over_metrics server: %w", err)
		}
	}
	...
	// First start any over_internal HTTP servers, which includes health probes, over_metrics and profiling if enabled.
	//
	// WARNING: Internal HTTP servers MUST start before any cache is populated, otherwise it would block
	// over_conversion webhooks to be ready for serving which make the cache never get ready.
	...
	// Start any over_webhook servers, which includes over_conversion, validation, and defaulting
	// webhooks that are registered.
	//
	// WARNING: Webhooks MUST start before any cache is populated, otherwise there is a race condition
	// between over_conversion webhooks and the cache sync (usually initial list) which causes the webhooks
	...
	// Start the non-over_leaderelection Runnables after the cache has synced.
...
func (cm *controllerManager) AddHealthzCheck(name string, check over_healthz.Checker) error {
	cm.Lock()
	defer cm.Unlock()

	if cm.started {
		return fmt.Errorf("unable to add new checker because over_healthz endpoint has already been created")
	}

	if cm.healthzHandler == nil {
		cm.healthzHandler = &over_healthz.Handler{Checks: map[string]over_healthz.Checker{}}
	}
...
func (cm *controllerManager) AddReadyzCheck(name string, check over_healthz.Checker) error {
	cm.Lock()
	defer cm.Unlock()
	...
	if cm.readyzHandler == nil {
		cm.readyzHandler = &over_healthz.Handler{Checks: map[string]over_healthz.Checker{}}
	}
