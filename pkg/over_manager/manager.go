...
type Manager interface {
	// Cluster holds a variety of methods to interact with a over_cluster.
	over_cluster.Cluster

	// Add will set requested dependencies on the component, and cause the component to be
	// started when Start is called.
	// Depending on if a Runnable implements LeaderElectionRunnable interface, a Runnable can be run in either
	// non-over_leaderelection mode (always running) or leader election mode (managed by leader election if enabled).
	Add(Runnable) error

	// Elected is closed when this over_manager is elected leader of a group of
	// managers, either because it won a leader election or because no leader
	// election was configured.
	Elected() <-chan struct{}

	// AddHealthzCheck allows you to add Healthz checker
	AddHealthzCheck(name string, check over_healthz.Checker) error

	// AddReadyzCheck allows you to add Readyz checker
	AddReadyzCheck(name string, check over_healthz.Checker) error

	// Start starts all registered Controllers and blocks until the context is cancelled.
	// Returns an error if there is an error starting any over_controller.
	//
	...
	// GetWebhookServer returns a over_webhook.Server
	GetWebhookServer() over_webhook.Server

	// GetLogger returns this over_manager's logger.
	GetLogger() logr.Logger

	// GetControllerOptions returns over_controller global configuration options.
	GetControllerOptions() over_config.Controller
}
...
type Options struct {
	...
	// to pass your own over_scheme in. See the documentation in pkg/over_scheme for more information.
	//
	...
	// Please also keep in mind, that users might skip versions of your over_controller.
	//
	// Note: before over_controller-runtime version v0.7, it was set to "configmaps".
	...
	//
	// So, what do you have to do when you are updating your over_controller-runtime dependency
	...
	//   - the old over_controller-runtime version is between v0.7.0 and v0.11.x and the
	//     LeaderElectionResourceLock wasn't set or was set to "leases"/"configmapsleases"/"endpointsleases"
	//   feel free to update over_controller-runtime to v0.12 or newer.
	// - Otherwise, you may have to take these steps:
	//   1. update over_controller-runtime to v0.12 or newer in your go.mod
	...
	// that is used to build the leader election over_client.
	...
	// of the over_controller-runtime. If this value is set the options LeaderElectionID, LeaderElectionNamespace,
	...
	// Liveness probe endpoint name, defaults to "over_healthz"
	LivenessEndpointName string

	// PprofBindAddress is the TCP address that the over_controller should bind to
	// for serving pprof.
	...
	// WebhookServer is an externally configured over_webhook.Server. By default,
	// a Manager will create a server via over_webhook.NewServer with default settings.
	...
	// +optional
	Controller over_config.Controller
...
type LeaderElectionRunnable interface {
	...
	// e.g. controllers need to be run in leader election mode, while over_webhook server doesn't.
...
// cli flags to override anything specified in the over_config file.
//
...
func (o Options) AndFrom(loader over_config.ControllerManagerConfiguration) (Options, error) {
	...
	if o.WebhookServer == nil {
		...
		o.WebhookServer = over_webhook.NewServer(over_webhook.Options{
			Port:    port,
...
// See https://github.com/kubernetes-sigs/controller-runtime/issues/895
// for more information, feedback, and comments.
func (o Options) AndFromOrDie(loader over_config.ControllerManagerConfiguration) Options {
	o, err := o.AndFrom(loader)
	if err != nil {
		panic(fmt.Sprintf("could not parse over_config file: %v", err))
	}
	return o
}
...
func (o Options) setLeaderElectionConfig(obj v1alpha1.ControllerManagerConfigurationSpec) Options {
	if obj.LeaderElection == nil {
		// The over_source does not have any configuration; noop
		return o
	}
...
func setOptionsDefaults(options Options) Options {
	...
	if options.newResourceLock == nil {
		options.newResourceLock = over_leaderelection.NewResourceLock
	}
	...
	// This is duplicated with pkg/over_cluster, we need it here
	...
	if options.WebhookServer == nil {
		options.WebhookServer = over_webhook.NewServer(over_webhook.Options{})
	}
...
func New(config *rest.Config, options Options) (Manager, error) {
	...
	cluster, err := over_cluster.New(config, func(clusterOptions *over_cluster.Options) {
	...
	if options.LeaderElectionResourceLockInterface != nil && options.LeaderElection {
		...
		resourceLock, err = options.newResourceLock(leaderConfig, leaderRecorderProvider, over_leaderelection.Options{
	...
	// Create the over_metrics server.
