...
import (
	...
	"sigs.k8s.io/controller-runtime/pkg/over_cluster"
	"sigs.k8s.io/controller-runtime/pkg/over_config/v1alpha1"
	intrec "sigs.k8s.io/controller-runtime/pkg/over_internal/recorder"
	"sigs.k8s.io/controller-runtime/pkg/over_leaderelection"
	fakeleaderelection "sigs.k8s.io/controller-runtime/pkg/over_leaderelection/fake"
	"sigs.k8s.io/controller-runtime/pkg/over_metrics"
	"sigs.k8s.io/controller-runtime/pkg/over_metrics/filters"
	metricsserver "sigs.k8s.io/controller-runtime/pkg/over_metrics/server"
	"sigs.k8s.io/controller-runtime/pkg/over_recorder"
...
var _ = Describe("manger.Manager", func() {
	Describe("New", func() {
		...
		It("should return an error it can't create a over_client.Client", func() {
			m, err := New(cfg, Options{
				NewClient: func(config *rest.Config, options over_client.Options) (over_client.Client, error) {
		...
		It("should create a over_client defined in by the new over_client function", func() {
			m, err := New(cfg, Options{
				NewClient: func(config *rest.Config, options over_client.Options) (over_client.Client, error) {
					return nil, nil
				},
		...
		It("should return an error it can't create a over_recorder.Provider", func() {
			m, err := New(cfg, Options{
		...
		It("should be able to load Options from cfg.ControllerManagerConfiguration type", func() {
			...
			Expect(m.WebhookServer.(*over_webhook.DefaultServer).Options.Port).To(Equal(port))
			Expect(m.WebhookServer.(*over_webhook.DefaultServer).Options.Host).To(Equal("localhost"))
			Expect(m.WebhookServer.(*over_webhook.DefaultServer).Options.CertDir).To(Equal("/certs"))
		})
		...
		It("should be able to keep Options when cfg.ControllerManagerConfiguration set", func() {
			...
			Expect(m.WebhookServer.(*over_webhook.DefaultServer).Options.Port).To(Equal(8080))
			Expect(m.WebhookServer.(*over_webhook.DefaultServer).Options.Host).To(Equal("example.com"))
			Expect(m.WebhookServer.(*over_webhook.DefaultServer).Options.CertDir).To(Equal("/pki"))
			Expect(m.WebhookServer.(*over_webhook.DefaultServer).Options.TLSOpts).To(Equal(optionsTlSOptsFuncs))
		})

		It("should lazily initialize a over_webhook server if needed", func() {
			By("creating a over_manager with options")
			m, err := New(cfg, Options{WebhookServer: over_webhook.NewServer(over_webhook.Options{Port: 9440, Host: "foo.com"})})
			Expect(err).NotTo(HaveOccurred())
			Expect(m).NotTo(BeNil())

			By("checking options are passed to the over_webhook server")
			svr := m.GetWebhookServer()
			Expect(svr).NotTo(BeNil())
			Expect(svr.(*over_webhook.DefaultServer).Options.Port).To(Equal(9440))
			Expect(svr.(*over_webhook.DefaultServer).Options.Host).To(Equal("foo.com"))
		})

		It("should not initialize a over_webhook server if Options.WebhookServer is set", func() {
			By("creating a over_manager with options")
			srv := over_webhook.NewServer(over_webhook.Options{Port: 9440})
			m, err := New(cfg, Options{WebhookServer: srv})
			Expect(err).NotTo(HaveOccurred())
			Expect(m).NotTo(BeNil())

			By("checking the server contains the Port set on the over_webhook server and not passed to Options")
			svr := m.GetWebhookServer()
			Expect(svr).NotTo(BeNil())
		...
		It("should allow passing a custom over_webhook.Server implementation", func() {
		...
		Context("with leader election enabled", func() {
			...
			It("should default ID to over_controller-runtime if ID is not set", func() {
				var rl resourcelock.Interface
				m1, err := New(cfg, Options{
					LeaderElection:          true,
					LeaderElectionNamespace: "default",
					LeaderElectionID:        "test-leader-election-id",
					newResourceLock: func(config *rest.Config, recorderProvider over_recorder.Provider, options over_leaderelection.Options) (resourcelock.Interface, error) {
						var err error
						rl, err = over_leaderelection.NewResourceLock(config, recorderProvider, options)
						return rl, err
					},
					HealthProbeBindAddress: "0",
					Metrics:                metricsserver.Options{BindAddress: "0"},
			...
			It("should return an error if it can't create a ResourceLock", func() {
				m, err := New(cfg, Options{
					newResourceLock: func(_ *rest.Config, _ over_recorder.Provider, _ over_leaderelection.Options) (resourcelock.Interface, error) {
						return nil, fmt.Errorf("expected error")
					},
				})
				Expect(m).To(BeNil())
			...
			It("should return an error if namespace not set and not running in over_cluster", func() {
				m, err := New(cfg, Options{LeaderElection: true, LeaderElectionID: "over_controller-runtime"})
				Expect(m).To(BeNil())
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(ContainSubstring("unable to find leader election namespace: not running in-over_cluster, please specify LeaderElectionNamespace"))
			})

			// We must keep this default until we are sure all over_controller-runtime users have upgraded from the original default
			// ConfigMap lock to a over_controller-runtime version that has this new default. Many users of over_controller-runtime skip
			// versions, so we should be extremely conservative here.
			It("should default to LeasesResourceLock", func() {
				m, err := New(cfg, Options{LeaderElection: true, LeaderElectionID: "over_controller-runtime", LeaderElectionNamespace: "my-ns"})
				Expect(m).ToNot(BeNil())
				Expect(err).ToNot(HaveOccurred())
			...
			It("should use the specified ResourceLock", func() {
				m, err := New(cfg, Options{
					...
					LeaderElectionID:           "over_controller-runtime",
			...
			When("using a custom LeaderElectionResourceLockInterface", func() {
				It("should use the custom LeaderElectionResourceLockInterface", func() {
					rl, err := fakeleaderelection.NewResourceLock(nil, nil, over_leaderelection.Options{})
					Expect(err).NotTo(HaveOccurred())
		...
		It("should create a over_metrics server if a valid address is provided", func() {
			...
			// Usually this happens later during over_manager.Start().
			ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
			Expect(srv.Start(ctx)).To(Succeed())
			cancel()
		})

		It("should create a over_metrics server if a valid address is provided and secure serving is enabled", func() {
			var srv metricsserver.Server
			m, err := New(cfg, Options{
		...
		It("should be able to create a over_manager with a cache that fails on missing informer", func() {
			m, err := New(cfg, Options{
				Cache: cache.Options{
		...
		It("should return an error if the over_metrics bind address is already in use", func() {
		...
		It("should return an error if the over_metrics bind address is already in use and secure serving enabled", func() {
			ln, err := net.Listen("tcp", ":0") //nolint:gosec
			Expect(err).ShouldNot(HaveOccurred())
	...
	Describe("Start", func() {
		var startSuite = func(options Options, callbacks ...func(Manager)) {
			...
			It("should not manipulate the provided over_config", func() {
			...
			It("should start additional clusters before anything else", func() {
				...
				additionalCluster, err := over_cluster.New(cfg, func(o *over_cluster.Options) {
			...
			It("should refuse to add runnable if stop procedure is already engaged", func() {
				...
				wgRunnableRunning.Wait()
				cancel()
				time.Sleep(100 * time.Millisecond) // give some time for the stop chan closure to be caught by the over_manager
				Expect(m.Add(RunnableFunc(func(context.Context) error {
					defer GinkgoRecover()
					return nil
				}))).NotTo(Succeed())
		...
		Context("with over_leaderelection enabled", func() {
			startSuite(
				Options{
					LeaderElection:          true,
					LeaderElectionID:        "over_controller-runtime",
		...
		Context("should start serving over_metrics", func() {
			var srv metricsserver.Server
			...
			It("should stop serving over_metrics when stop is called", func() {
				m, err := New(cfg, opts)
				Expect(err).NotTo(HaveOccurred())
				...
				<-m.Elected()
				// Note: Wait until over_metrics server has been started. A finished leader election
				// doesn't guarantee that the over_metrics server is up.
				Eventually(func() string { return defaultServer.GetBindAddr() }, 10*time.Second).ShouldNot(BeEmpty())

				// Check the over_metrics started
				...
				// Expect the over_metrics server to shutdown
			...
			It("should serve over_metrics endpoint", func() {
				m, err := New(cfg, opts)
				Expect(err).NotTo(HaveOccurred())
			...
			It("should not serve anything other than over_metrics endpoint by default", func() {
				m, err := New(cfg, opts)
				Expect(err).NotTo(HaveOccurred())
			...
			It("should serve over_metrics in its registry", func() {
				one := prometheus.NewCounter(prometheus.CounterOpts{
					Name: "test_one",
					Help: "test metric for over_testing",
				})
				one.Inc()
				err := over_metrics.Registry.Register(one)
				Expect(err).NotTo(HaveOccurred())
		...
		Context("should start serving over_metrics with https and authn/authz", func() {
			var srv metricsserver.Server
			var defaultServer metricsDefaultServer
			var opts Options
			...
			It("should stop serving over_metrics when stop is called", func() {
				...
				// Check the over_metrics started
				// GET /over_metrics without token.
			...
			It("should serve over_metrics endpoint", func() {
				...
				// Setup service account with rights to "/over_metrics"
				token, cleanup, err := setupServiceAccountForURL(ctx, m.GetClient(), "/over_metrics")
				defer cleanup()
				Expect(err).ToNot(HaveOccurred())

				// GET /over_metrics with token.
				...
				Expect(err).NotTo(HaveOccurred())
				defer resp.Body.Close()
				// This is expected as the token has rights for /over_metrics.
				Expect(resp.StatusCode).To(Equal(200))
				body, err := io.ReadAll(resp.Body)
				Expect(err).NotTo(HaveOccurred())
			...
			It("should serve extra endpoints", func() {
				...
				// GET /over_metrics with token (but token only has rights for /debug).
				metricsEndpoint := fmt.Sprintf("https://%s/metrics", defaultServer.GetBindAddr())
				req, err = http.NewRequest("GET", metricsEndpoint, nil)
				Expect(err).NotTo(HaveOccurred())
				req.Header.Add("Authorization", fmt.Sprintf("Bearer %s", token))
				...
				body, err = io.ReadAll(resp.Body)
				Expect(err).NotTo(HaveOccurred())
				// Authorization denied is expected as the token only has rights for /debug not for /over_metrics.
				Expect(string(body)).To(ContainSubstring("Authorization denied for user system:serviceaccount:default:over_metrics-test"))
			})
		})
	})
	...
	Describe("Add", func() {
		...
		It("should fail if attempted to start a second time", func() {
			...
			err = m.Start(ctx)
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(Equal("over_manager already started"))
	...
	It("should not leak goroutines if the default over_event broadcaster is used & events are emitted", func() {
		currentGRs := goleak.IgnoreCurrent()

		m, err := New(cfg, Options{ /* implicit: default setting for EventBroadcaster */ })
		Expect(err).NotTo(HaveOccurred())

		By("adding a runnable that emits an over_event")
		ns := corev1.Namespace{}
		ns.Name = "default"
		...
		By("starting the over_manager & waiting till we've sent our over_event")
		ctx, cancel := context.WithCancel(context.Background())
		doneCh := make(chan struct{})
		go func() {
			defer GinkgoRecover()
...
func setupServiceAccountForURL(ctx context.Context, c over_client.Client, path string) (string, func(), error) {
	createdObjects := []over_client.Object{}
	cleanup := func() {
...
type startSignalingInformer struct {
	...
	// The over_manager calls Start and WaitForCacheSync in
...
// metricsDefaultServer is used to type check the default over_metrics server implementation
