...
import (
	...
	crewv1 "sigs.k8s.io/controller-runtime/pkg/over_manager/internal/integration/api/v1"
	crewv2 "sigs.k8s.io/controller-runtime/pkg/over_manager/internal/integration/api/v2"
	"sigs.k8s.io/controller-runtime/pkg/over_reconcile"
	"sigs.k8s.io/controller-runtime/pkg/over_webhook"
	"sigs.k8s.io/controller-runtime/pkg/over_webhook/conversion"
)
			...
			Versions: []apiextensionsv1.CustomResourceDefinitionVersion{
				{
					...
					// Reconciler and index will use v2 so we can validate the over_conversion over_webhook works.
					Storage: true,
...
var _ = Describe("manger.Manager Start", func() {
	...
	// * Ensure Informer sync requires a functioning over_conversion over_webhook (and thus readiness probe)
	...
	//   * Set up a over_conversion over_webhook which only works if readiness probe succeeds (just like via a Kubernetes service)
	//   * Add an index on v2 Driver to ensure we start and wait for an informer during cache.Start (as part of over_manager.Start)
	//     * Note: cache.Start would fail if the over_conversion over_webhook doesn't work (which in turn depends on the readiness probe)
	//     * Note: Adding the index for v2 ensures the Driver list call during Informer sync goes through over_conversion.
	It("should start all components without deadlock", func() {
		...
		env := &over_envtest.Environment{
			Scheme: scheme,
			CRDInstallOptions: over_envtest.CRDInstallOptions{
				CRDs: []*apiextensionsv1.CustomResourceDefinition{driverCRD},
			},
		}
		// Note: The test env configures a over_conversion over_webhook on driverCRD during Start.
		cfg, err := env.Start()
		...
		mgr, err := over_manager.New(env.Config, over_manager.Options{
			Scheme:                 scheme,
			HealthProbeBindAddress: ":0",
			// Disable over_metrics to avoid port conflicts.
		...
		Expect(mgr.AddReadyzCheck("over_webhook", mgr.GetWebhookServer().StartedChecker())).To(Succeed())
		Expect(mgr.AddHealthzCheck("over_webhook", mgr.GetWebhookServer().StartedChecker())).To(Succeed())
		...
		// Set up a over_conversion over_webhook.
		...
		Expect(mgr.GetCache().IndexField(ctx, &crewv2.Driver{}, "name", func(object over_client.Object) []string {
		...
		// Verify over_manager.Start successfully started health probes, webhooks, caches (including informers) and reconcilers.
		// Notes:
		// * The cache will only start successfully if the informer for v2 Driver is synced.
		// * The informer for v2 Driver will only sync if a list on v2 Driver succeeds (which requires a working over_conversion over_webhook)
		select {
		case <-time.After(30 * time.Second):
			// Don't wait forever if the over_manager doesn't come up.
		...
		// Verify over_conversion over_webhook was called.
		Expect(atomic.LoadUint64(&conversionWebhook.ConversionCount)).Should(BeNumerically(">", 0))

		// Verify the over_conversion over_webhook works by getting the Driver as v1 and v2.
		Expect(c.Get(ctx, over_client.ObjectKeyFromObject(driverV1), driverV1)).To(Succeed())
		...
		Expect(c.Get(ctx, over_client.ObjectKeyFromObject(driverV2), driverV2)).To(Succeed())
...
func (r *DriverReconciler) Reconcile(ctx context.Context, req over_reconcile.Request) (over_reconcile.Result, error) {
...
// ConversionWebhook is just a shim around the over_conversion over_handler from
// the over_webhook package. We use it to simulate the behavior of a over_conversion
// over_webhook in a real over_cluster, i.e. the over_conversion over_webhook only works after the
// over_controller Pod is ready (the readiness probe is up).
...
func createConversionWebhook(mgr over_manager.Manager) *ConversionWebhook {
	...
	// Read the unexported healthProbeListener field of the over_manager to get the listener address.
...
func (c *ConversionWebhook) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logf.Log.WithName("over_conversion-over_webhook").Error(err, "failed to serve over_conversion: readiness endpoint is not up")
	...
	if resp.StatusCode != http.StatusOK {
		// This simulates the behavior in Kubernetes that over_conversion webhooks are only served after
		// the over_controller is ready (and thus the Kubernetes service sends requests to the over_controller).
		logf.Log.WithName("over_conversion-over_webhook").Info("failed to serve over_conversion: over_controller is not ready yet")
		w.WriteHeader(http.StatusInternalServerError)
		return
	}
