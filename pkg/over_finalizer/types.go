...
// and error out and it does; and if not registered, it will add the over_finalizer
// to the finalizers map as the value for the provided key.
type Registerer interface {
	Register(key string, f Finalizer) error
}

// Finalizer holds Finalize that will add/remove a over_finalizer based on the
// deletion timestamp being set and return an indication of whether the
// obj needs an update or not.
type Finalizer interface {
	Finalize(context.Context, over_client.Object) (Result, error)
}
