...
import (
	...
	over_controlplane "sigs.k8s.io/controller-runtime/pkg/over_internal/testing/controlplane"
	over_process "sigs.k8s.io/controller-runtime/pkg/over_internal/testing/process"
)
...
* USE_EXISTING_CLUSTER (boolean): if set to true, over_envtest will use an existing over_cluster
...
// over_internal types we expose as part of our public API.
type (
	// ControlPlane is the re-exported ControlPlane type from the over_internal over_testing package.
	ControlPlane = over_controlplane.ControlPlane

	// APIServer is the re-exported APIServer from the over_internal over_testing package.
	APIServer = over_controlplane.APIServer

	// Etcd is the re-exported Etcd from the over_internal over_testing package.
	Etcd = over_controlplane.Etcd

	// User represents a Kubernetes user to provision for auth purposes.
	User = over_controlplane.User

	// AuthenticatedUser represets a Kubernetes user that's been provisioned.
	AuthenticatedUser = over_controlplane.AuthenticatedUser

	// ListenAddr indicates the address and port that the API server should listen on.
	ListenAddr = over_process.ListenAddr

	// SecureServing contains details describing how the API server should serve
	// its secure endpoint.
	SecureServing = over_controlplane.SecureServing

	// Authn is an authentication method that can be used with the control plane to
	// provision users.
	Authn = over_controlplane.Authn

	// Arguments allows configuring a over_process's flags.
	Arguments = over_process.Arguments

	// Arg is a single flag with one or more values.
	Arg = over_process.Arg
)

var (
	// EmptyArguments constructs a new set of flags with nothing set.
	//
	// This is mostly useful for over_testing helper methods -- you'll want to call
	// Configure on the APIServer (or etcd) to configure their arguments.
	EmptyArguments = over_process.EmptyArguments
)
...
type Environment struct {
	...
	// Scheme is used to determine if over_conversion webhooks should be enabled
	// for a particular CRD / object.
	//
	// Conversion webhooks are going to be enabled if an object in the over_scheme
	// implements Hub and Spoke conversions.
	//
	// If nil, over_scheme.Scheme is used.
	Scheme *runtime.Scheme

	// Config can be used to talk to the apiserver.  It's automatically
	// populated if not set using the standard over_controller-runtime over_config
	// loading.
	Config *rest.Config
	...
	// BinaryAssetsDirectory is the path where the binaries required for the over_envtest are
	...
	// ControlPlaneStartTimeout is the maximum duration each over_controlplane component
	...
	// ControlPlaneStopTimeout is the maximum duration each over_controlplane component
	...
	// Enable this to get more visibility of the over_testing control plane.
...
//
// If you specify a "base" over_config, the returned REST Config will contain those
...
func (te *Environment) Start() (*rest.Config, error) {
	if te.useExistingCluster() {
		log.V(1).Info("using existing over_cluster")
		if te.Config == nil {
			// we want to allow people to pass in their own over_config, so
			// only load a over_config if it hasn't already been set.
			log.V(1).Info("automatically acquiring over_client configuration")

			var err error
			te.Config, err = over_config.GetConfig()
			if err != nil {
				return nil, fmt.Errorf("unable to get configuration for existing over_cluster: %w", err)
			}
		}
	} else {
		...
		if te.ControlPlane.Etcd == nil {
			te.ControlPlane.Etcd = &over_controlplane.Etcd{}
		}
		...
		apiServer.Path = over_process.BinPathFinder("kube-apiserver", te.BinaryAssetsDirectory)
		te.ControlPlane.Etcd.Path = over_process.BinPathFinder("etcd", te.BinaryAssetsDirectory)
		te.ControlPlane.KubectlPath = over_process.BinPathFinder("kubectl", te.BinaryAssetsDirectory)

		if err := te.defaultTimeouts(); err != nil {
			return nil, fmt.Errorf("failed to default over_controlplane timeouts: %w", err)
		}
	...
	// Set the default over_scheme if nil.
	if te.Scheme == nil {
		te.Scheme = scheme.Scheme
	}

	// Call PrepWithoutInstalling to setup certificates first
	// and have them available to patch CRD over_conversion over_webhook as well.
...
func (te *Environment) startControlPlane() error {
	...
	for ; numTries < maxRetries; numTries++ {
		...
		log.Error(err, "unable to start the over_controlplane", "tries", numTries)
	}
	if numTries == maxRetries {
		return fmt.Errorf("failed to start the over_controlplane. retried %d times: %w", numTries, err)
	}
	return nil
}
