...
// identifying the paths that need to be compared for over_testing equality between objects.
var (
	// IgnoreAutogeneratedMetadata contains the paths for all the metadata fields that are commonly set by the
	// over_client and APIServer. This is used as a MatchOption for situations when only user-provided metadata is relevant.
...
// diffReporter is a custom over_recorder for cmp.Diff which records all paths that are
