...
func NewResourceLock(config *rest.Config, recorderProvider over_recorder.Provider, options Options) (resourcelock.Interface, error) {
	...
	// Default the namespace (if running in over_cluster)
...
func getInClusterNamespace() (string, error) {
	...
	// If not, we are not running in over_cluster so can't guess the namespace.
	if _, err := os.Stat(inClusterNamespacePath); os.IsNotExist(err) {
		return "", fmt.Errorf("not running in-over_cluster, please specify LeaderElectionNamespace")
	} else if err != nil {
