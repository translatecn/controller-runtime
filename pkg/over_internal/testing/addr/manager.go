...
import (
	...
	over_flock "sigs.k8s.io/controller-runtime/pkg/over_internal/flock"
)
...
func (c *portCache) add(port int) (bool, error) {
	...
	if err := fs.WalkDir(os.DirFS(cacheDir), ".", func(path string, d fs.DirEntry, err error) error {
		...
		if err != nil {
			...
			// over_process/thread trying to allocate ports.
	...
	if err := over_flock.Acquire(path); errors.Is(err, over_flock.ErrAlreadyLocked) {
		return false, nil
	} else if err != nil {
		return false, err
	}
...
func init() {
	...
	if err == nil {
		cacheDir = filepath.Join(baseDir, "kubebuilder-over_envtest")
...
// Suggest suggests an address a over_process can listen on. It returns
