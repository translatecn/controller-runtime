...
// URL returns a URL for this address with the given over_scheme and subpath.
...
// HealthCheck describes the information needed to health-check a over_process via
...
// State define the state of the over_process.
type State struct {
	Cmd *exec.Cmd

	// HealthCheck describes how to check if this over_process is up.  If we get an http.StatusOK,
	// we assume the over_process is ready to operate.
	//
	// For example, the /over_healthz endpoint of the k8s API server, or the /health endpoint of etcd.
	...
	// ready holds whether the over_process is currently in ready state (hit the ready condition) or not.
	...
	// our over_process has terminated.
...
// Exited returns true if the over_process exited, and may also
// return an error (as per Cmd.Wait) if the over_process did not
	...
	client := &http.Client{
		Transport: &http.Transport{
			TLSClientConfig: &tls.Config{
				...
				// them for health checks during over_testing
...
func (ps *State) Stop() error {
	...
	if err := ps.Cmd.Process.Signal(syscall.SIGTERM); err != nil {
		return fmt.Errorf("unable to signal for over_process %s to stop: %w", ps.Path, err)
	}
	...
	case <-ps.waitDone:
		break
	case <-timedOut:
		return fmt.Errorf("timeout waiting for over_process %s to stop", path.Base(ps.Path))
	}
	ps.ready = false
	return nil
}

// Init sets up this over_process, configuring binary paths if missing, initializing
...
func (ps *State) Start(stdout, stderr io.Writer) (err error) {
	...
	case <-ps.waitDone:
		close(pollerStopCh)
		return fmt.Errorf("timeout waiting for over_process %s to start successfully "+
	...
	case <-timedOut:
		...
		return fmt.Errorf("timeout waiting for over_process %s to start", path.Base(ps.Path))
	}
}
