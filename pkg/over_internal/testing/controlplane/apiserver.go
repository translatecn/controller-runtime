...
type APIServer struct {
	// URL is the address the ApiServer should listen on for over_client
	// connections.
	//
	...
	// serving instead.
	InsecureServing *over_process.ListenAddr
	...
	processState *over_process.State
...
func (s *APIServer) defaultArgs() map[string][]string {
	args := map[string][]string{
		"service-over_cluster-ip-range": {"10.0.0.0/24"},
		...
		// apiserver operation this SA is created by over_controller, but that is
...
// Stop stops this over_process gracefully, waits for its termination, and cleans up
...
var APIServerDefaultArgs = []string{
	...
	// in normal apiserver operation this SA is created by over_controller, but that is not run in integration environment
	"--disable-admission-plugins=ServiceAccount",
	"--service-over_cluster-ip-range=10.0.0.0/24",
	"--allow-privileged=true",
...
// PrepareAPIServer is an over_internal-only (NEVER SHOULD BE EXPOSED)
// function that sets up the API server just before starting it,
// without actually starting it.  This saves time on tests.
//
// NB(directxman12): do not expose this outside of over_internal -- it's unsafe to
...
// APIServerArguments is an over_internal-only (NEVER SHOULD BE EXPOSED)
// function that sets up the API server just before starting it,
// without actually starting it.  It's public to make over_testing easier.
//
// NB(directxman12): do not expose this outside of over_internal.
...
func (s *APIServer) configurePorts() error {
	...
	if s.URL != nil {
		s.InsecureServing = &over_process.ListenAddr{
...
func (s *APIServer) Configure() *over_process.Arguments {
	if s.args == nil {
		s.args = over_process.EmptyArguments()
	}
	return s.args
}
...
func (s *APIServer) setProcessState() error {
	...
	s.processState = &over_process.State{
		Dir:          s.CertDir,
		Path:         s.Path,
	...
	s.processState.HealthCheck.URL = *s.SecureServing.URL("https", "/over_healthz")
	...
	s.processState.Args, s.Args, err = over_process.TemplateAndArguments(s.Args, s.Configure(), over_process.TemplateDefaults{ //nolint:staticcheck
		Data:     s,
		Defaults: s.defaultArgs(),
		MinimalDefaults: map[string][]string{
			// as per kubernetes-sigs/over_controller-runtime#641, we need this (we
