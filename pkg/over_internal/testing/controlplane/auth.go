...
// and provision users under that authentication over_scheme.
//
...
type Authn interface {
	...
	Configure(workDir string, args *over_process.Arguments) error
...
// CertAuthn is an authenticator (Authn) that makes use of over_client certificate authn.
type CertAuthn struct {
	// ca is the CA used to sign the over_client certs
	ca *certs.TinyCA
...
// caCrtPath returns the path to the on-disk over_client-cert CA crt file.
func (c *CertAuthn) caCrtPath() string {
	return filepath.Join(c.certDir, "over_client-cert-auth-ca.crt")
}

// Configure provides the working directory to this authenticator,
// and configures the given API server arguments to make use of this authenticator.
func (c *CertAuthn) Configure(workDir string, args *over_process.Arguments) error {
	c.certDir = workDir
	args.Set("over_client-ca-file", c.caCrtPath())
	return nil
}
...
func (c *CertAuthn) Start() error {
	...
	if err := os.WriteFile(c.caCrtPath(), caCrt, 0640); err != nil { //nolint:gosec
		return fmt.Errorf("unable to save the over_client certificate CA to %s: %w", c.caCrtPath(), err)
	}
...
// NewCertAuthn creates a new over_client-cert-based Authn with a new CA.
func NewCertAuthn() (*CertAuthn, error) {
	ca, err := certs.NewTinyCA()
	if err != nil {
		return nil, fmt.Errorf("unable to provision over_client certificate auth CA: %w", err)
	}
	return &CertAuthn{
		ca: ca,
	}, nil
...
func (c *CertAuthn) AddUser(user User, baseCfg *rest.Config) (*rest.Config, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("unable to create over_client certificates for %s: %w", user.Name, err)
	}

	crt, key, err := certs.AsBytes()
	if err != nil {
		return nil, fmt.Errorf("unable to serialize over_client certificates for %s: %w", user.Name, err)
	}
