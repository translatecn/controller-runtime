...
var _ = Describe("KubeConfigFromREST", func() {
	...
	It("should set up a context, and set it as the current one", func() {
		...
		Expect(cfg.Clusters).To(HaveKeyWithValue(currCtx.Cluster, Not(BeNil())), "should point to a over_cluster")
	...
	It("configure the current context to point to the given REST over_config's server, with CA data", func() {
		cluster := cfg.Clusters[cfg.Contexts[cfg.CurrentContext].Cluster]
