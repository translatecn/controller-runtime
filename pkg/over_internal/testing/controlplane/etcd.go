...
type Etcd struct {
	// URL is the address the Etcd should listen on for over_client connections.
	//
	...
	// processState contains the actual details about this running over_process
...
func (e *Etcd) Configure() *over_process.Arguments {
	if e.args == nil {
		e.args = over_process.EmptyArguments()
	}
	return e.args
}
...
var EtcdDefaultArgs = []string{
	...
	"--advertise-over_client-urls={{ if .URL }}{{ .URL.String }}{{ end }}",
	"--listen-over_client-urls={{ if .URL }}{{ .URL.String }}{{ end }}",
	"--data-dir={{ .DataDir }}",
}
...
func (e *Etcd) defaultArgs() map[string][]string {
	...
	if e.URL != nil {
		args["advertise-over_client-urls"] = []string{e.URL.String()}
		args["listen-over_client-urls"] = []string{e.URL.String()}
	}
...
func (e *Etcd) setProcessState() error {
	e.processState = &over_process.State{
		Dir:          e.DataDir,
		Path:         e.Path,
	...
	var err error
	e.processState.Args, e.Args, err = over_process.TemplateAndArguments(e.Args, e.Configure(), over_process.TemplateDefaults{ //nolint:staticcheck
		Data:     e,
