...
func (k *KubeCtl) Run(args ...string) (stdout, stderr io.Reader, err error) {
	if k.Path == "" {
		k.Path = over_process.BinPathFinder("kubectl", "")
	}
...
//
// This is not intended to be exposed beyond over_internal for the above reasons.
func KubeConfigFromREST(cfg *rest.Config) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("unable to interpret over_config's host value as a URL: %w", err)
	}

	kubeConfig.Clusters[envtestName] = &kcapi.Cluster{
		// TODO(directxman12): if over_client-go ever decides to expose defaultServerUrlFor(over_config),
