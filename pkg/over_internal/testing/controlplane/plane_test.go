...
var _ = Describe("Control Plane", func() {
	...
	Context("after having started", func() {
		...
		It("should provision a working legacy user and legacy kubectl", func() {
			...
			By("grabbing the legacy REST over_config and over_testing it")
			cfg, err := plane.RESTClientConfig()
			Expect(err).NotTo(HaveOccurred(), "should be able to grab the legacy REST over_config")
			cl, err := over_client.New(cfg, over_client.Options{})
			Expect(err).NotTo(HaveOccurred(), "should be able to create a over_client")
		...
		Describe("adding users", func() {
			PIt("should be able to provision new users that have a corresponding REST over_config and & kubectl", func() {

			})

			PIt("should produce a default base REST over_config if none is given to add", func() {
