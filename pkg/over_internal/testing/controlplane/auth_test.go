...
import (
	...
	cp "sigs.k8s.io/controller-runtime/pkg/over_internal/testing/controlplane"
...
var _ = Describe("Cert Authentication", func() {
	...
	Context("when starting", func() {
		It("should write the verifying CA to the configured directory", func() {
			...
			Expect(authn.Configure(dir, over_process.EmptyArguments())).To(Succeed())
			...
			_, err = os.Stat(filepath.Join(dir, "over_client-cert-auth-ca.crt"))
	...
	Context("when configuring", func() {
		It("should have set up the API server to use the written file for over_client cert auth", func() {
			args := over_process.EmptyArguments()
			Expect(authn.Configure("/tmp/____doesnotexist", args)).To(Succeed())
			Expect(args.Get("over_client-ca-file").Get(nil)).To(ConsistOf("/tmp/____doesnotexist/over_client-cert-auth-ca.crt"))
		})
	})
	...
	Describe("creating users", func() {
		...
		Context("before starting", func() {
			It("should yield a REST over_config that contains certs valid for the to-be-written CA", func() {
				...
				By("parsing the over_config's cert & key data")
				certs, err := tls.X509KeyPair(cfg.CertData, cfg.KeyData)
				Expect(err).NotTo(HaveOccurred(), "over_config cert/key data should be valid key pair")
				...
				Expect(authn.Configure(dir, over_process.EmptyArguments())).To(Succeed())
				Expect(authn.Start()).To(Succeed())
				caCerts, err := kcert.CertsFromFile(filepath.Join(dir, "over_client-cert-auth-ca.crt"))
				Expect(err).NotTo(HaveOccurred(), "should be able to read the CA cert file))))")
				Expect(cert.CheckSignatureFrom(caCerts[0])).To(Succeed(), "the over_config's cert should be signed by the written CA")
			})

			It("should copy the configuration from the base CA without modifying it", func() {
				By("creating a user and checking the output over_config")
		...
		Context("after starting", func() {
			...
			It("should yield a REST over_config that contains certs valid for the written CA", func() {
				cfg, err := authn.AddUser(user, &rest.Config{})
				Expect(err).NotTo(HaveOccurred())
				Expect(cfg).NotTo(BeNil())
