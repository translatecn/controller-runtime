...
import (
	...
	overprocess "sigs.k8s.io/controller-runtime/pkg/over_internal/testing/process"
)
	...
	Describe("setting up serving hosts & ports", func() {
		Context("when URL is set", func() {
			...
			Context("when insecure serving is also set", func() {
				BeforeEach(func() {
					server.InsecureServing = &overprocess.ListenAddr{
						Address: "localhost",
						Port:    "1234",
					}
				})
				...
				It("should override the existing insecure serving", func() {
					Expect(server.InsecureServing).To(Equal(&overprocess.ListenAddr{
						Address: "localhost",
						Port:    "8675",
					}))
				})
		...
		Context("when given legacy-style template arguments", func() {
			...
			It("should use the passed in args with the minimal required defaults", func() {
				Expect(APIServerArguments(server)).To(ConsistOf(
					...
					MatchRegexp("--over_client-ca-file=.+"),
...
func (f *fakeAuthn) Configure(workDir string, args *overprocess.Arguments) error {
	f.workDir = workDir
	if f.setFlag {
