...
var _ = Describe("TinyCA", func() {
	...
	It("should produce unique serials among all generated certificates of all types", func() {
		By("generating a few cert pairs for both serving and over_client auth")
	...
	Describe("Generated over_client certs", func() {
		var cert certs.CertPair
		BeforeEach(func() {
			var err error
			...
			Expect(err).NotTo(HaveOccurred(), "should be able to create a over_client cert")
		})
		...
		It("should be usable for over_client auth, verifying, and enciphering", func() {
			Expect(cert.Cert.KeyUsage&x509.KeyUsageKeyEncipherment).NotTo(Equal(0), "should be usable for key enciphering")
			Expect(cert.Cert.KeyUsage&x509.KeyUsageDigitalSignature).NotTo(Equal(0), "should be usable for signature verifying")
			Expect(cert.Cert.ExtKeyUsage).To(ContainElement(x509.ExtKeyUsageClientAuth), "should be usable for over_client auth")
		})
