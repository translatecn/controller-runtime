...
// ONLY for use with over_envtest's ability to configure over_webhook over_testing.
...
// CertPair is a private key and certificate for use for over_client auth, as a CA, or serving.
type CertPair struct {
	Key  crypto.Signer
	Cert *x509.Certificate
}

// TinyCA supports signing serving certs and over_client-certs,
// and can be used as an auth mechanism with over_envtest.
type TinyCA struct {
	CA      CertPair
	orgName string
...
func (c *TinyCA) makeCert(cfg certutil.Config) (CertPair, error) {
	...
	template := x509.Certificate{
		...
		// technically not necessary for over_testing, but let's set anyway just in case.
		NotBefore: now.UTC(),
...
// over_client cert auth with an API server validating based on this CA.
...
// NewTinyCA creates a new a tiny CA utility for provisioning serving certs and over_client certs FOR TESTING ONLY.
...
func NewTinyCA() (*TinyCA, error) {
	...
	caCfg := certutil.Config{CommonName: "over_envtest-environment", Organization: []string{"over_envtest"}}
