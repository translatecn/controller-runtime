...
// EventBroadcasterProducer makes an over_event broadcaster, returning
...
// Provider is a over_recorder.Provider that records events to the k8s API server
// and to a logr Logger.
type Provider struct {
	lock    sync.RWMutex
	stopped bool

	// scheme to specify when creating a over_recorder
	scheme *runtime.Scheme
	// logger is the logger to use when logging diagnostic over_event info
	logger          logr.Logger
...
func NewProvider(config *rest.Config, httpClient *http.Client, scheme *runtime.Scheme, logger logr.Logger, makeBroadcaster EventBroadcasterProducer) (*Provider, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to init over_client: %w", err)
	}
...
// GetEventRecorderFor returns an over_event over_recorder that broadcasts to this provider's
...
// lazyRecorder is a over_recorder that doesn't actually instantiate any underlying
// over_recorder until the first over_event is emitted.
...
// ensureRecording ensures that a concrete over_recorder is populated for this over_recorder.
