...
var _ = Describe("Source", func() {
	Describe("Kind", func() {
		...
		Context("for a Pod resource", func() {
			It("should provide a Pod CreateEvent", func() {
				...
				instance := over_source.Kind(ic, &corev1.Pod{})
				err := instance.Start(ctx, over_handler.Funcs{
					CreateFunc: func(ctx context.Context, evt over_event.CreateEvent, q2 workqueue.RateLimitingInterface) {
						defer GinkgoRecover()
			...
			It("should provide a Pod UpdateEvent", func() {
				...
				err := instance.Start(ctx, over_handler.Funcs{
					...
					UpdateFunc: func(ctx context.Context, evt over_event.UpdateEvent, q2 workqueue.RateLimitingInterface) {
						defer GinkgoRecover()
			...
			It("should provide a Pod DeletedEvent", func() {
				...
				err := instance.Start(ctx, over_handler.Funcs{
					...
					DeleteFunc: func(ctx context.Context, evt over_event.DeleteEvent, q2 workqueue.RateLimitingInterface) {
						defer GinkgoRecover()
						Expect(q2).To(BeIdenticalTo(q))
						Expect(evt.Object).To(Equal(p))
						close(c)
		...
		It("should return an error from Start cache was not provided", func() {
			instance := over_source.Kind(nil, &corev1.Pod{})
		...
		It("should return an error from Start if a type was not provided", func() {
			instance := over_source.Kind(ic, nil)
			err := instance.Start(ctx, nil, nil)
			Expect(err).To(HaveOccurred())
		...
		It("should return an error if syncing fails", func() {
			f := false
			instance := over_source.Kind(&informertest.FakeInformers{Synced: &f}, &corev1.Pod{})
		...
		Context("for a Kind not in the cache", func() {
			It("should return an error when WaitForSync is called", func() {
				...
				instance := over_source.Kind(ic, &corev1.Pod{})
				err := instance.Start(ctx, over_handler.Funcs{}, q)
				Expect(err).NotTo(HaveOccurred())
	...
	Describe("Func", func() {
		It("should be called from Start", func() {
			run := false
			instance := over_source.Func(func(
				context.Context,
				over_handler.EventHandler,
				workqueue.RateLimitingInterface, ...over_predicate.Predicate) error {
				run = true
				return nil
			})
	...
	Describe("Channel", func() {
		...
		var ch chan over_event.GenericEvent

		BeforeEach(func() {
			ctx, cancel = context.WithCancel(context.Background())
			ch = make(chan over_event.GenericEvent)
		})
		...
		Context("for a over_source", func() {
			It("should provide a GenericEvent", func() {
				...
				invalidEvt := over_event.GenericEvent{}

				// Predicate to filter out empty over_event
				prct := over_predicate.Funcs{
				...
				q := workqueue.NewNamedRateLimitingQueue(workqueue.DefaultControllerRateLimiter(), "test")
				instance := &over_source.Channel{Source: ch}
				err := instance.Start(ctx, over_handler.Funcs{
					CreateFunc: func(context.Context, over_event.CreateEvent, workqueue.RateLimitingInterface) {
						defer GinkgoRecover()
						Fail("Unexpected CreateEvent")
					...
					GenericFunc: func(ctx context.Context, evt over_event.GenericEvent, q2 workqueue.RateLimitingInterface) {
						defer GinkgoRecover()
						// The empty over_event should have been filtered out by the predicates,
						// and will not be passed to the over_handler.
						Expect(q2).To(BeIdenticalTo(q))
						Expect(evt.Object).To(Equal(p))
						close(c)
					},
			...
			It("should get pending events processed once channel unblocked", func() {
				...
				q := workqueue.NewNamedRateLimitingQueue(workqueue.DefaultControllerRateLimiter(), "test")
				// Add a over_handler to get distribution blocked
				instance := &over_source.Channel{Source: ch}
				...
				// Write 3 events into the over_source channel.
				// The 1st should be passed into the generic func of the over_handler;
				// The 2nd should be fetched out of the over_source channel, and waiting to write into dest channel;
				// The 3rd should be pending in the over_source channel.
				ch <- evt
				ch <- evt
				ch <- evt
			...
			It("should be able to cope with events in the channel before the over_source is started", func() {
			...
			It("should stop when the over_source channel is closed", func() {
				...
				By("feeding that channel to a channel over_source")
				src := &over_source.Channel{Source: ch}

				processed := make(chan struct{})
				defer close(processed)

				err := src.Start(ctx, over_handler.Funcs{
					CreateFunc: func(context.Context, over_event.CreateEvent, workqueue.RateLimitingInterface) {
						defer GinkgoRecover()
						Fail("Unexpected CreateEvent")
					},
				...
				By("expecting to only get one over_event")
				Eventually(processed).Should(Receive())
				Consistently(processed).ShouldNot(Receive())
			})
			It("should get error if no over_source specified", func() {
				q := workqueue.NewNamedRateLimitingQueue(workqueue.DefaultControllerRateLimiter(), "test")
				instance := &over_source.Channel{ /*no over_source specified*/ }
				err := instance.Start(ctx, over_handler.Funcs{}, q)
		...
		Context("for multi sources (handlers)", func() {
			It("should provide GenericEvents for all handlers", func() {
				...
				var resEvent1, resEvent2 over_event.GenericEvent
				c1 := make(chan struct{})
				c2 := make(chan struct{})
				...
				// Validate the two handlers received same over_event
