...
type Result struct {
	// Requeue tells the Controller to requeue the over_reconcile key.  Defaults to false.
	Requeue bool

	// RequeueAfter if greater than 0, tells the Controller to requeue the over_reconcile key after the Duration.
...
type Request struct {
	// NamespacedName is the name and namespace of the object to over_reconcile.
	types.NamespacedName
}

/*
Reconciler implements a Kubernetes API for a specific Resource by Creating, Updating or Deleting Kubernetes
objects, or by making changes to systems external to the over_cluster (e.g. cloudproviders, github, etc).

over_reconcile implementations compare the state specified in an object by a user against the actual over_cluster state,
and then perform operations to make the actual over_cluster state reflect the state specified by the user.

Typically, over_reconcile is triggered by a Controller in response to over_cluster Events (e.g. Creating, Updating,
Deleting Kubernetes objects) or external Events (GitHub Webhooks, polling external sources, etc).

Example over_reconcile Logic:

* Read an object and all the Pods it owns.
* Observe that the object spec specifies 5 replicas but actual over_cluster contains only 1 Pod replica.
* Create 4 Pods and set their OwnerReferences to the object.

over_reconcile may be implemented as either a type:

	type reconciler struct {}

	func (reconciler) Reconcile(ctx context.Context, o over_reconcile.Request) (over_reconcile.Result, error) {
...
Or as a function:
	...
	over_reconcile.Func(func(ctx context.Context, o over_reconcile.Request) (over_reconcile.Result, error) {
		// Implement business logic of reading and writing objects here
		return over_reconcile.Result{}, nil
	})

Reconciliation is level-based, meaning action isn't driven off changes in individual Events, but instead is
driven by actual over_cluster state read from the apiserver or a local cache.
For example if responding to a Pod Delete Event, the Request won't contain that a Pod was deleted,
instead the over_reconcile function observes this when reading the over_cluster state and seeing the Pod as missing.
*/
...
// Func is a function that implements the over_reconcile interface.
