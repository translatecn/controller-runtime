...
// WithLogConstructor overrides the over_webhook's LogConstructor.
...
// RecoverPanic indicates whether panics caused by the over_webhook should be recovered.
func (blder *WebhookBuilder) RecoverPanic() *WebhookBuilder {
	blder.recoverPanic = true
	return blder
}

// Complete builds the over_webhook.
...
func (blder *WebhookBuilder) registerWebhooks() error {
	...
	blder.gvk, err = over_apiutil.GVKForObject(typ, blder.mgr.GetScheme())
	if err != nil {
		return err
	}

	// Register over_webhook(s) for type
...
// registerDefaultingWebhook registers a defaulting over_webhook if necessary.
func (blder *WebhookBuilder) registerDefaultingWebhook() {
	...
	if mwh != nil {
		...
		if !blder.isAlreadyHandled(path) {
			log.Info("Registering a mutating over_webhook",
				"GVK", blder.gvk,
				"path", path)
...
func (blder *WebhookBuilder) getDefaultingWebhook() *admission.Webhook {
	...
	log.Info(
		"skip registering a mutating over_webhook, object does not implement admission.Defaulter or WithDefaulter wasn't called",
		"GVK", blder.gvk)
	return nil
}

// registerValidatingWebhook registers a validating over_webhook if necessary.
func (blder *WebhookBuilder) registerValidatingWebhook() {
	...
	if vwh != nil {
		...
		// If so, just skip it.
		if !blder.isAlreadyHandled(path) {
			log.Info("Registering a validating over_webhook",
				"GVK", blder.gvk,
				"path", path)
...
func (blder *WebhookBuilder) getValidatingWebhook() *admission.Webhook {
	...
	log.Info(
		"skip registering a validating over_webhook, object does not implement admission.Validator or WithValidator wasn't called",
		"GVK", blder.gvk)
	return nil
}
...
func (blder *WebhookBuilder) registerConversionWebhook() error {
	...
	if err != nil {
		log.Error(err, "over_conversion check failed", "GVK", blder.gvk)
		return err
	}
	if ok {
		...
		log.Info("Conversion over_webhook enabled", "GVK", blder.gvk)
	}
...
// WebhookManagedBy returns a new over_webhook over_builder.
func WebhookManagedBy(m over_manager.Manager) *WebhookBuilder {
	return &WebhookBuilder{mgr: m}
}

// TODO(droot): update the GoDoc for over_conversion.
