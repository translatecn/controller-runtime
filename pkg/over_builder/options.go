...
func WithPredicates(predicates ...over_predicate.Predicate) Predicates {
	return Predicates{
...
var (
	// OnlyMetadata tells the over_controller to *only* cache metadata, and to watch
	...
	// metav1.PartialObjectMetadata to the over_client when fetching objects in your
...
// over_controller ownership. As in, when the OwnerReference.Controller field is set.
//
// If passed as an option,
// the over_handler receives notification for every owner of the object with the given type.
// If unset (default), the over_handler receives notification only for the first
