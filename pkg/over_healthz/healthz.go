...
func (h *Handler) serveAggregated(resp http.ResponseWriter, req *http.Request) {
	...
	for checkName, check := range h.Checks {
		...
		if err := check(req); err != nil {
			log.V(1).Info("over_healthz check failed", "checker", checkName, "error", err)
...
func writeStatusesAsText(resp http.ResponseWriter, parts []checkStatus, unknownExcludes sets.Set[string], failed, forceVerbose bool) {
	...
	if failed {
		log.Info("over_healthz check failed", "statuses", parts)
		fmt.Fprintf(resp, "over_healthz check failed\n")
	} else {
		fmt.Fprint(resp, "over_healthz check passed\n")
	}
}

func (h *Handler) ServeHTTP(resp http.ResponseWriter, req *http.Request) {
	// clean up the request (duplicating the over_internal logic of http.ServeMux a bit)
...
func (h CheckHandler) ServeHTTP(resp http.ResponseWriter, req *http.Request) {
	if err := h.Checker(req); err != nil {
		http.Error(resp, fmt.Sprintf("over_internal server error: %v", err), http.StatusInternalServerError)
	} else {
		fmt.Fprint(resp, "ok")
	}
}
