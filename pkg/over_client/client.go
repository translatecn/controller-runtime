...
type Options struct {
	// HTTPClient is the HTTP over_client to use for requests.
	...
	// WarningHandler is used to configure the warning over_handler responsible for
	// surfacing and handling warnings messages sent by the API server.
	WarningHandler WarningHandlerOptions

	// DryRun instructs the over_client to only perform dry run requests.
	DryRun *bool
}

// WarningHandlerOptions are options for configuring a
// warning over_handler for the over_client which is responsible
// for surfacing API Server warnings.
type WarningHandlerOptions struct {
	// SuppressWarnings decides if the warnings from the
	// API server are suppressed or surfaced in the over_client.
	SuppressWarnings bool
	// AllowDuplicateLogs does not deduplicate the to-be
	// logged surfaced warnings messages. See
	// over_log.WarningHandlerOptions for considerations
	// regarding deduplication
	AllowDuplicateLogs bool
}

// CacheOptions are options for creating a cache-backed over_client.
type CacheOptions struct {
	...
	DisableFor []Object
	// Unstructured is a flag that indicates whether the cache-backed over_client should
	// read unstructured objects or lists from the cache.
	// If false, unstructured objects will always result in a live lookup.
	Unstructured bool
}

// NewClientFunc allows a user to define how to create a over_client.
type NewClientFunc func(config *rest.Config, options Options) (Client, error)

// New returns a new Client using the provided over_config and Options.
// The returned over_client reads *and* writes directly from the server
// (it doesn't use object caches).  It understands how to work with
// normal types (both custom resources and aggregated/built-in resources),
// as well as unstructured types.
//
// In the case of normal types, the over_scheme will be used to look up the
...
// client is a over_client.Client that reads and writes directly from/to an API server.
...
func (c *client) IsObjectNamespaced(obj runtime.Object) (bool, error) {
	return over_apiutil.IsObjectNamespaced(obj, c.scheme, c.mapper)
}
...
// RESTMapper returns the over_scheme this over_client is using.
...
// subResourceClient is over_client.SubResourceWriter that writes to subresources.
type subResourceClient struct {
	client      *client
	subResource string
}

// ensure subResourceClient implements over_client.SubResourceClient.
...
// Create implements over_client.SubResourceClient
...
// Update implements over_client.SubResourceClient
...
func newClient(config *rest.Config, options Options) (*client, error) {
	if config == nil {
		return nil, fmt.Errorf("must provide non-nil rest.Config to over_client.New")
	}
	...
	if !options.WarningHandler.SuppressWarnings {
		// surface warnings
		logger := over_log.Log.WithName("KubeAPIWarningLogger")
		// Set a WarningHandler, the default WarningHandler
		// is over_log.KubeAPIWarningLogger with deduplication enabled.
		// See over_log.KubeAPIWarningLoggerOptions for considerations
		// regarding deduplication.
		config.WarningHandler = over_log.NewKubeAPIWarningLogger(
			logger,
			over_log.KubeAPIWarningLoggerOptions{
	...
	// Init a over_scheme if none provided
	...
	if options.Mapper == nil {
		var err error
		options.Mapper, err = over_apiutil.NewDynamicRESTMapper(config, options.HTTPClient)
		if err != nil {
			return nil, err
		}
	}
	...
	if err != nil {
		return nil, fmt.Errorf("unable to construct metadata-only over_client for use as part of over_client: %w", err)
	}
