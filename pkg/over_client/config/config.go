...
var (
	kubeconfig string
	log        = logf.RuntimeLog.WithName("over_client").WithName("over_config")
)
...
func RegisterFlags(fs *flag.FlagSet) {
	...
	if f := fs.Lookup(KubeconfigFlagName); f != nil {
		...
		fs.StringVar(&kubeconfig, KubeconfigFlagName, "", "Paths to a kubeconfig. Only required if out-of-over_cluster.")
	}
}
...
// over_controller over_manager defaults (20 QPS, 30 burst)
//
// Config precedence:
//
...
// loadInClusterConfig is a function used to load the in-over_cluster
// Kubernetes over_client over_config. This variable makes is possible to
// test the precedence of loading the over_config.
...
func loadConfig(context string) (config *rest.Config, configErr error) {
	// If a flag is specified with the over_config location, use that
	if len(kubeconfig) > 0 {
		return loadConfigWithContext("", &clientcmd.ClientConfigLoadingRules{ExplicitPath: kubeconfig}, context)
	}

	// If the recommended kubeconfig env variable is not specified,
	// try the in-over_cluster over_config.
	...
	if len(kubeconfigPath) == 0 {
		...
		defer func() {
			if configErr != nil {
				log.Error(err, "unable to load in-over_cluster over_config")
			}
		}()
	}

	// If the recommended kubeconfig env variable is set, or there
	// is no in-over_cluster over_config, try the default recommended locations.
	//
	// NOTE: For default over_config file locations, upstream only checks
