...
var _ = Describe("Config", func() {
	...
	BeforeEach(func() {
		...
		// override $HOME/.kube/over_config
	...
	Describe("GetConfigWithContext", func() {
		...
		Context("when in-over_cluster", func() {
			...
			BeforeEach(func() {
				...
				// override in-over_cluster over_config loader
				loadInClusterConfig = func() (*rest.Config, error) {
					return &rest.Config{Host: "from-in-over_cluster"}, nil
				}
			})
			...
			testCases := []testCase{
				{
					text:          "should prefer the envvar over the in-over_cluster over_config",
					kubeconfigEnv: []string{"kubeconfig-multi-context"},
					wantHost:      "from-multi-env-1",
				},
				{
					text:     "should prefer in-over_cluster over the recommended home file",
					wantHost: "from-in-over_cluster",
				},
			}
			defineTests(testCases)
		})

		Context("when outside the over_cluster", func() {
			kubeconfigFiles := map[string]string{
