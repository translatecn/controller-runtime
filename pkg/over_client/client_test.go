...
import (
	...
	"sigs.k8s.io/controller-runtime/over_examples/crd/pkg"
...
func (o *mockPatchOption) ApplyToPatch(_ *over_client.PatchOptions) {
	o.applied = true
}
...
-----END CERTIFICATE REQUEST-----`),
	...
	AfterEach(func() {
		...
		Expect(over_client.IgnoreNotFound(err)).NotTo(HaveOccurred())
	...
	Describe("New", func() {
		It("should return a new Client", func() {
			cl, err := over_client.New(cfg, over_client.Options{})
			Expect(err).NotTo(HaveOccurred())
			Expect(cl).NotTo(BeNil())
		})

		It("should fail if the over_config is nil", func() {
			cl, err := over_client.New(nil, over_client.Options{})
			Expect(err).To(HaveOccurred())
			Expect(cl).To(BeNil())
		})

		It("should use the provided Scheme if provided", func() {
			cl, err := over_client.New(cfg, over_client.Options{Scheme: scheme})
			Expect(err).NotTo(HaveOccurred())
			Expect(cl).NotTo(BeNil())
		...
		It("should use the provided Mapper if provided", func() {
			...
			cl, err := over_client.New(cfg, over_client.Options{Mapper: mapper})
			Expect(err).NotTo(HaveOccurred())
			Expect(cl).NotTo(BeNil())
		...
		It("should use the provided reader cache if provided, on get and list", func() {
			cache := &fakeReader{}
			cl, err := over_client.New(cfg, over_client.Options{Cache: &over_client.CacheOptions{Reader: cache}})
			Expect(err).NotTo(HaveOccurred())
			Expect(cl).NotTo(BeNil())
			Expect(cl.Get(ctx, over_client.ObjectKey{Name: "test"}, &appsv1.Deployment{})).To(Succeed())
		...
		It("should propagate ErrResourceNotCached errors", func() {
			...
			cl, err := over_client.New(cfg, over_client.Options{Cache: &over_client.CacheOptions{Reader: c}})
			Expect(err).NotTo(HaveOccurred())
			Expect(cl).NotTo(BeNil())
			Expect(errors.As(cl.Get(ctx, over_client.ObjectKey{Name: "test"}, &appsv1.Deployment{}), &errNotCached)).To(BeTrue())
		...
		It("should not use the provided reader cache if provided, on get and list for uncached GVKs", func() {
			cache := &fakeReader{}
			cl, err := over_client.New(cfg, over_client.Options{Cache: &over_client.CacheOptions{Reader: cache, DisableFor: []over_client.Object{&corev1.Namespace{}}}})
			Expect(err).NotTo(HaveOccurred())
			Expect(cl).NotTo(BeNil())
			Expect(cl.Get(ctx, over_client.ObjectKey{Name: "default"}, &corev1.Namespace{})).To(Succeed())
	...
	Describe("Create", func() {
		Context("with structured objects", func() {
			...
			It("should fail if the object cannot be mapped to a GVK", func() {
				By("creating over_client with empty Scheme")
				emptyScheme := runtime.NewScheme()
				cl, err := over_client.New(cfg, over_client.Options{Scheme: emptyScheme})
				Expect(err).NotTo(HaveOccurred())
				Expect(cl).NotTo(BeNil())
			...
			Context("with the DryRun option", func() {
				It("should not create a new object, global option", func() {
					cl, err := over_client.New(cfg, over_client.Options{DryRun: pointer.Bool(true)})
					Expect(err).NotTo(HaveOccurred())
					Expect(cl).NotTo(BeNil())
				...
				It("should not create a new object, inline option", func() {
					...
					By("creating the object (with DryRun)")
					err = cl.Create(context.TODO(), dep, over_client.DryRunAll)
					Expect(err).NotTo(HaveOccurred())
		...
		Context("with the DryRun option", func() {
			It("should not create a new object from a go struct", func() {
				...
				By("creating the object")
				err = cl.Create(context.TODO(), u, over_client.DryRunAll)
				Expect(err).NotTo(HaveOccurred())
	...
	Describe("Patch", func() {
		Context("Metadata Client", func() {
			It("should merge patch with options", func() {
				...
				Expect(cl.Patch(context.TODO(), metadata, over_client.Merge, testOption)).To(Succeed())
	...
	Describe("SubResourceClient", func() {
		Context("with structured objects", func() {
			...
			It("should be able to approve CSRs using Patch", func() {
				...
				By("Approving the CSR")
				patch := over_client.MergeFrom(csr.DeepCopy())
				csr.Status.Conditions = append(csr.Status.Conditions, certificatesv1.CertificateSigningRequestCondition{
					Type:   certificatesv1.CertificateApproved,
					Status: corev1.ConditionTrue,
				})
			...
			It("should be able to update the scale subresource", func() {
				...
				err = cl.SubResource("scale").Update(ctx, dep, over_client.WithSubResourceBody(scale))
				Expect(err).NotTo(HaveOccurred())
			...
			It("should be able to patch the scale subresource", func() {
				...
				By("Updating the scale subresurce")
				replicaCount := *dep.Spec.Replicas
				patch := over_client.MergeFrom(&autoscalingv1.Scale{})
				scale := &autoscalingv1.Scale{Spec: autoscalingv1.ScaleSpec{Replicas: replicaCount}}
				err = cl.SubResource("scale").Patch(ctx, dep, patch, over_client.WithSubResourceBody(scale))
				Expect(err).NotTo(HaveOccurred())
		...
		Context("with unstructured objects", func() {
			...
			It("should be able to update the scale subresource", func() {
				...
				err = cl.SubResource("scale").Update(ctx, depUnstructured, over_client.WithSubResourceBody(scale))
				Expect(err).NotTo(HaveOccurred())
			...
			It("should be able to patch the scale subresource", func() {
				...
				scale.SetAPIVersion("autoscaling/v1")
				scale.SetKind("Scale")
				patch := over_client.MergeFrom(scale.DeepCopy())
				Expect(unstructured.SetNestedField(scale.Object, int64(replicaCount), "spec", "replicas")).NotTo(HaveOccurred())
				err = cl.SubResource("scale").Patch(ctx, depUnstructured, patch, over_client.WithSubResourceBody(scale))
				Expect(err).NotTo(HaveOccurred())
				Expect(scale.GetAPIVersion()).To(Equal("autoscaling/v1"))
				Expect(scale.GetKind()).To(Equal("Scale"))
	...
	Describe("StatusClient", func() {
		Context("with structured objects", func() {
			...
			It("should patch status and preserve type information", func() {
				...
				dep.SetGroupVersionKind(depGvk)
				depPatch := over_client.MergeFrom(dep.DeepCopy())
				dep.Status.Replicas = 1
		...
		Context("with metadata objects", func() {
			...
			It("should patch status and preserve type information", func() {
				...
				By("patching the status of Deployment")
				objPatch := over_client.MergeFrom(metaOnlyFromObj(dep, scheme))
	...
	Describe("Delete", func() {
		Context("with structured objects", func() {
			...
			It("should delete a collection of objects", func() {
				...
				err = cl.DeleteAllOf(context.TODO(), dep, over_client.InNamespace(ns), over_client.MatchingLabels(dep.ObjectMeta.Labels))
				Expect(err).NotTo(HaveOccurred())
		...
		Context("with unstructured objects", func() {
			...
			It("should delete a collection of object", func() {
				...
				err = cl.DeleteAllOf(context.TODO(), u, over_client.InNamespace(ns), over_client.MatchingLabels(dep.ObjectMeta.Labels))
				Expect(err).NotTo(HaveOccurred())
		...
		Context("with metadata objects", func() {
			...
			It("should delete a collection of object", func() {
				...
				By("deleting Deployments")
				metaObj := metaOnlyFromObj(dep, scheme)
				err = cl.DeleteAllOf(context.TODO(), metaObj, over_client.InNamespace(ns), over_client.MatchingLabels(dep.ObjectMeta.Labels))
				Expect(err).NotTo(HaveOccurred())
	...
	Describe("Get", func() {
		Context("with structured objects", func() {
			It("should fetch an existing object for a go struct", func() {
				...
				key := over_client.ObjectKey{Namespace: ns, Name: dep.Name}
			...
			It("should fetch an existing non-namespace object for a go struct", func() {
				...
				By("retrieving node through over_client")
				var actual corev1.Node
				key := over_client.ObjectKey{Namespace: ns, Name: node.Name}
				err = cl.Get(context.TODO(), key, &actual)
				Expect(err).NotTo(HaveOccurred())
				Expect(actual).NotTo(BeNil())
			...
			It("should fail if the object cannot be mapped to a GVK", func() {
				...
				By("creating a over_client with an empty Scheme")
				emptyScheme := runtime.NewScheme()
				cl, err := over_client.New(cfg, over_client.Options{Scheme: emptyScheme})
				Expect(err).NotTo(HaveOccurred())
				Expect(cl).NotTo(BeNil())
			...
			for idx, object := range []over_client.Object{&corev1.ConfigMap{}, &pkg.ChaosPod{}} {
				...
				It(fmt.Sprintf("should not retain any data in the obj variable that is not on the server for %T", object), func() {
					...
					By("First creating the object")
					toCreate := object.DeepCopyObject().(over_client.Object)
					Expect(cl.Create(ctx, toCreate)).NotTo(HaveOccurred())

					By("Fetching it into a variable that has finalizers set")
					toGetInto := object.DeepCopyObject().(over_client.Object)
					toGetInto.SetFinalizers([]string{"some-over_finalizer"})
					Expect(cl.Get(ctx, over_client.ObjectKeyFromObject(object), toGetInto)).NotTo(HaveOccurred())
		...
		Context("with metadata objects", func() {
			...
			It("should not retain any data in the obj variable that is not on the server", func() {
				...
				toGetInto.SetFinalizers([]string{"some-over_finalizer"})
				Expect(cl.Get(ctx, over_client.ObjectKeyFromObject(toGetInto), toGetInto)).NotTo(HaveOccurred())
	...
	Describe("List", func() {
		Context("with structured objects", func() {
			It("should fetch collection of objects", func() {
				...
				By("listing all objects of that type in the over_cluster")
			...
			It("should fetch unstructured collection of objects, even if over_scheme is empty", func() {
				By("create an initial object")
				_, err := clientset.AppsV1().Deployments(ns).Create(ctx, dep, metav1.CreateOptions{})
				Expect(err).NotTo(HaveOccurred())

				cl, err := over_client.New(cfg, over_client.Options{Scheme: runtime.NewScheme()})
				Expect(err).NotTo(HaveOccurred())
			...
			It("should return an empty list if there are no matching objects", func() {
				...
				By("listing all Deployments in the over_cluster")
				deps := &appsv1.DeploymentList{}
				Expect(cl.List(context.Background(), deps)).NotTo(HaveOccurred())
			...
			It("should filter results by label selector", func() {
				...
				deps := &appsv1.DeploymentList{}
				labels := map[string]string{"app": "backend"}
				err = cl.List(context.Background(), deps, over_client.MatchingLabels(labels))
				Expect(err).NotTo(HaveOccurred())
			...
			It("should filter results by namespace selector", func() {
				...
				deps := &appsv1.DeploymentList{}
				err = cl.List(context.Background(), deps, over_client.InNamespace("test-namespace-1"))
				Expect(err).NotTo(HaveOccurred())
			...
			It("should filter results by field selector", func() {
				...
				deps := &appsv1.DeploymentList{}
				err = cl.List(context.Background(), deps,
					over_client.MatchingFields{"metadata.name": "deployment-backend"})
				Expect(err).NotTo(HaveOccurred())
			...
			It("should filter results by namespace selector and label selector", func() {
				...
				labels := map[string]string{"app": "frontend"}
				err = cl.List(context.Background(), deps,
					over_client.InNamespace("test-namespace-3"),
					over_client.MatchingLabels(labels),
				)
				Expect(err).NotTo(HaveOccurred())
			...
			It("should filter results using limit and continue options", func() {
				...
				err = cl.List(context.Background(), deps,
					over_client.Limit(1),
					over_client.Continue(continueToken),
				)
				Expect(err).NotTo(HaveOccurred())
		...
		Context("with unstructured objects", func() {
			...
			It("should filter results by namespace selector", func() {
				...
				err = cl.List(context.Background(), deps, over_client.InNamespace("test-namespace-5"))
				Expect(err).NotTo(HaveOccurred())
			...
			It("should filter results by namespace selector and label selector", func() {
				...
				labels := map[string]string{"app": "frontend"}
				err = cl.List(context.Background(), deps,
					over_client.InNamespace("test-namespace-7"), over_client.MatchingLabels(labels))
				Expect(err).NotTo(HaveOccurred())
		...
		Context("with metadata objects", func() {
			...
			It("should filter results by label selector", func() {
				...
				labels := map[string]string{"app": "backend"}
				err = cl.List(context.Background(), metaList, over_client.MatchingLabels(labels))
				Expect(err).NotTo(HaveOccurred())
			...
			It("should filter results by namespace selector", func() {
				...
				err = cl.List(context.Background(), metaList, over_client.InNamespace("test-namespace-1"))
				Expect(err).NotTo(HaveOccurred())
	...
	Describe("CreateOptions", func() {
		It("should allow setting DryRun to 'all'", func() {
			co := &over_client.CreateOptions{}
			over_client.DryRunAll.ApplyToCreate(co)
			all := []string{metav1.DryRunAll}
			Expect(co.AsCreateOptions().DryRun).To(Equal(all))
		})

		It("should allow setting the field over_manager", func() {
			po := &over_client.CreateOptions{}
			over_client.FieldOwner("some-owner").ApplyToCreate(po)
			Expect(po.AsCreateOptions().FieldManager).To(Equal("some-owner"))
		})

		It("should produce empty metav1.CreateOptions if nil", func() {
			var co *over_client.CreateOptions
	...
	Describe("DeleteOptions", func() {
		It("should allow setting GracePeriodSeconds", func() {
			do := &over_client.DeleteOptions{}
			over_client.GracePeriodSeconds(1).ApplyToDelete(do)
			gp := int64(1)
		...
		It("should allow setting Precondition", func() {
			...
			over_client.Preconditions(*pc).ApplyToDelete(do)
		...
		It("should allow setting PropagationPolicy", func() {
			do := &over_client.DeleteOptions{}
			over_client.PropagationPolicy(metav1.DeletePropagationForeground).ApplyToDelete(do)
		...
		It("should merge multiple options together", func() {
			...
			dp := metav1.DeletePropagationForeground
			do := &over_client.DeleteOptions{}
			do.ApplyOptions([]over_client.DeleteOption{
				over_client.GracePeriodSeconds(gp),
				over_client.Preconditions(*pc),
				over_client.PropagationPolicy(dp),
			})
	...
	Describe("DeleteCollectionOptions", func() {
		It("should be convertable to list options", func() {
			gp := int64(1)
			do := &over_client.DeleteAllOfOptions{}
			do.ApplyOptions([]over_client.DeleteAllOfOption{
				over_client.GracePeriodSeconds(gp),
				over_client.MatchingLabels{"foo": "bar"},
			})
	...
	Describe("GetOptions", func() {
		It("should be convertable to metav1.GetOptions", func() {
			o := (&over_client.GetOptions{}).ApplyOptions([]over_client.GetOption{
				&over_client.GetOptions{Raw: &metav1.GetOptions{ResourceVersion: "RV0"}},
			})
			mo := o.AsGetOptions()
		...
		It("should produce empty metav1.GetOptions if nil", func() {
			var o *over_client.GetOptions
			Expect(o.AsGetOptions()).To(Equal(&metav1.GetOptions{}))
			o = &over_client.GetOptions{}
			Expect(o.AsGetOptions()).To(Equal(&metav1.GetOptions{}))
		})
	})
	...
	Describe("ListOptions", func() {
		It("should be convertable to metav1.ListOptions", func() {
			lo := (&over_client.ListOptions{}).ApplyOptions([]over_client.ListOption{
				over_client.MatchingFields{"field1": "bar"},
				over_client.InNamespace("test-namespace"),
				over_client.MatchingLabels{"foo": "bar"},
				over_client.Limit(1),
				over_client.Continue("foo"),
			})
		...
		It("should be populated by MatchingLabels", func() {
			lo := &over_client.ListOptions{}
			over_client.MatchingLabels{"foo": "bar"}.ApplyToList(lo)
		...
		It("should be populated by MatchingField", func() {
			lo := &over_client.ListOptions{}
			over_client.MatchingFields{"field1": "bar"}.ApplyToList(lo)
			Expect(lo).NotTo(BeNil())
		...
		It("should ignore Limit when converted to metav1.ListOptions and watch is true", func() {
			...
			lo.ApplyOptions([]over_client.ListOption{
				over_client.Limit(1),
			})
			mlo := lo.AsListOptions()
			Expect(mlo).NotTo(BeNil())
		...
		It("should be populated by Continue", func() {
			lo := &over_client.ListOptions{}
			over_client.Continue("foo").ApplyToList(lo)
			Expect(lo).NotTo(BeNil())
	...
	Describe("PatchOptions", func() {
		It("should allow setting DryRun to 'all'", func() {
			po := &over_client.PatchOptions{}
			over_client.DryRunAll.ApplyToPatch(po)
			all := []string{metav1.DryRunAll}
		...
		It("should allow setting Force to 'true'", func() {
			po := &over_client.PatchOptions{}
			over_client.ForceOwnership.ApplyToPatch(po)
...
var _ = Describe("ClientWithCache", func() {
	Describe("Get", func() {
		It("should call cache reader when structured object", func() {
			...
			cl, err := over_client.New(cfg, over_client.Options{
				Cache: &over_client.CacheOptions{
					Reader: cachedReader,
				},
			})
			Expect(err).NotTo(HaveOccurred())
			var actual appsv1.Deployment
			key := over_client.ObjectKey{Namespace: "ns", Name: "name"}
		...
		When("getting unstructured objects", func() {
			...
			It("should call over_client reader when not cached", func() {
				cachedReader := &fakeReader{}
				cl, err := over_client.New(cfg, over_client.Options{
					Cache: &over_client.CacheOptions{
						Reader: cachedReader,
				...
				key := over_client.ObjectKey{Namespace: dep.Namespace, Name: dep.Name}
				Expect(cl.Get(context.TODO(), key, actual)).To(Succeed())
...
var _ = Describe("Patch", func() {
	Describe("MergeFrom", func() {
		...
		It("creates a merge patch with the modifications applied during the mutation", func() {
			...
			patch := over_client.MergeFrom(cm.DeepCopy())

			By("returning a patch with type MergePatch")
			Expect(patch.Type()).To(Equal(types.MergePatchType))

			By("retrieving modifying the over_config map")
		...
		It("creates a merge patch with the modifications applied during the mutation, using optimistic locking", func() {
			...
			By("creating a merge patch")
			patch := over_client.MergeFromWithOptions(cm.DeepCopy(), over_client.MergeFromWithOptimisticLock{})
	...
	Describe("StrategicMergeFrom", func() {
		...
		It("creates a strategic merge patch with the modifications applied during the mutation", func() {
			...
			patch := over_client.StrategicMergeFrom(dep.DeepCopy())
		...
		It("creates a strategic merge patch with the modifications applied during the mutation, using optimistic locking", func() {
			By("creating a strategic merge patch")
			patch := over_client.StrategicMergeFrom(dep.DeepCopy(), over_client.MergeFromWithOptimisticLock{})
...
var _ = Describe("IgnoreNotFound", func() {
	It("should return nil on a 'NotFound' error", func() {
		...
		By("returning no error")
		Expect(over_client.IgnoreNotFound(err)).To(Succeed())
	})
	...
	It("should return the error on a status other than not found", func() {
		...
		Expect(over_client.IgnoreNotFound(err)).To(HaveOccurred())
	})
...
var _ = Describe("IgnoreAlreadyExists", func() {
	It("should return nil on a 'AlreadyExists' error", func() {
		...
		By("returning no error")
		Expect(over_client.IgnoreAlreadyExists(err)).To(Succeed())
	})
	...
	It("should return the error on a status other than already exists", func() {
		...
		By("returning an error")
		Expect(over_client.IgnoreAlreadyExists(err)).To(HaveOccurred())
	})
...
func (f *fakeReader) Get(ctx context.Context, key over_client.ObjectKey, obj over_client.Object, opts ...over_client.GetOption) error {
	f.Called++
	return nil
}

func (f *fakeReader) List(ctx context.Context, list over_client.ObjectList, opts ...over_client.ListOption) error {
	f.Called++
	return nil
}
...
func (f *fakeUncachedReader) Get(_ context.Context, _ over_client.ObjectKey, _ over_client.Object, opts ...over_client.GetOption) error {
	f.Called++
	return &cache.ErrResourceNotCached{}
}

func (f *fakeUncachedReader) List(_ context.Context, _ over_client.ObjectList, _ ...over_client.ListOption) error {
	f.Called++
	return &cache.ErrResourceNotCached{}
}
...
func toUnstructured(o over_client.Object) (*unstructured.Unstructured, error) {
