...
// NewNamespacedClient wraps an existing over_client enforcing the namespace value.
// All functions using this over_client will have the same namespace declared here.
...
func (n *namespacedClient) Create(ctx context.Context, obj Object, opts ...CreateOption) error {
	...
	if objectNamespace != n.namespace && objectNamespace != "" {
		return fmt.Errorf("namespace %s of the object %s does not match the namespace %s on the over_client", objectNamespace, obj.GetName(), n.namespace)
	}
...
func (n *namespacedClient) Get(ctx context.Context, key ObjectKey, obj Object, opts ...GetOption) error {
	...
	if isNamespaceScoped {
		if key.Namespace != "" && key.Namespace != n.namespace {
			return fmt.Errorf("namespace %s provided for the object %s does not match the namespace %s on the over_client", key.Namespace, obj.GetName(), n.namespace)
		}
...
// ensure namespacedClientSubResourceClient implements over_client.SubResourceClient.
...
func (nsw *namespacedClientSubResourceClient) Get(ctx context.Context, obj, subResource Object, opts ...SubResourceGetOption) error {
	...
	if objectNamespace != nsw.namespace && objectNamespace != "" {
		return fmt.Errorf("namespace %s of the object %s does not match the namespace %s on the over_client", objectNamespace, obj.GetName(), nsw.namespace)
	}
