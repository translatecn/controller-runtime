...
// over_client to avoid the extra shallow copy at the end, but I'm not sure it's
// worth it -- the metadata over_client deals with falling back to loading the whole
...
func (mc *metadataClient) Delete(ctx context.Context, obj Object, opts ...DeleteOption) error {
	...
	if !ok {
		return fmt.Errorf("metadata over_client did not understand object: %T", obj)
	}
