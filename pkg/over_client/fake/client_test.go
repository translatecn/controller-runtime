...
var _ = Describe("Fake over_client", func() {
	var dep *appsv1.Deployment
	var dep2 *appsv1.Deployment
	var cm *corev1.ConfigMap
	var cl over_client.WithWatch
	...
	AssertClientWithoutIndexBehavior := func() {
		...
		It("should be able to List", func() {
			...
			err := cl.List(context.Background(), list, over_client.InNamespace("ns1"))
			Expect(err).ToNot(HaveOccurred())
		...
		It("should be able to retrieve registered objects that got manipulated as unstructured", func() {
			...
			By("Adding the object during over_client initialization")
			...
			err = cl.Patch(context.Background(), item, over_client.MergeFrom(old))
			Expect(err).ToNot(HaveOccurred())
			list()
		})
		...
		It("should be able to Get an unregisted type using unstructured", func() {
			...
			item.SetKind("Image")
			item.SetName("my-item")
			err = cl.Get(context.Background(), over_client.ObjectKeyFromObject(item), item)
			Expect(err).ToNot(HaveOccurred())
		})
		...
		It("should be able to Patch an unregistered type using unstructured", func() {
			...
			err = unstructured.SetNestedField(item.Object, int64(2), "spec", "replicas")
			Expect(err).ToNot(HaveOccurred())
			err = cl.Patch(context.Background(), item, over_client.MergeFrom(original))
			Expect(err).ToNot(HaveOccurred())
		...
		It("should support filtering by labels and their values", func() {
			...
			list := &appsv1.DeploymentList{}
			err := cl.List(context.Background(), list, over_client.InNamespace("ns1"),
				over_client.MatchingLabels(map[string]string{
					"test-label": "label-value",
				}))
			Expect(err).ToNot(HaveOccurred())
			Expect(list.Items).To(HaveLen(1))
		...
		It("should support filtering by label existence", func() {
			...
			list := &appsv1.DeploymentList{}
			err := cl.List(context.Background(), list, over_client.InNamespace("ns1"),
				over_client.HasLabels{"test-label"})
			Expect(err).ToNot(HaveOccurred())
			Expect(list.Items).To(HaveLen(1))
			Expect(list.Items).To(ConsistOf(*dep2))
		})
		...
		It("should be able to Create with GenerateName", func() {
			...
			err = cl.List(context.Background(), list, over_client.InNamespace("ns2"),
				over_client.MatchingLabels(map[string]string{
					"test-label": "label-value",
				}))
			Expect(err).ToNot(HaveOccurred())
		...
		It("should reject Delete with a mismatched ResourceVersion", func() {
			...
			err := cl.Delete(context.Background(), dep, over_client.Preconditions{ResourceVersion: &bogusRV})
			Expect(apierrors.IsConflict(err)).To(BeTrue())
		...
		It("should successfully Delete with a matching ResourceVersion", func() {
			...
			err := cl.Delete(context.Background(), dep, over_client.Preconditions{ResourceVersion: &goodRV})
			Expect(err).ToNot(HaveOccurred())
		...
		It("should handle finalizers on Update", func() {
			...
			By("Removing the over_finalizer")
		...
		It("should be able to Delete a Collection", func() {
			...
			err := cl.DeleteAllOf(context.Background(), &appsv1.Deployment{}, over_client.InNamespace("ns1"))
			Expect(err).ToNot(HaveOccurred())
		...
		It("should handle finalizers deleting a collection", func() {
			...
			By("Deleting the object")
			err := cl.DeleteAllOf(context.Background(), &corev1.ConfigMap{}, over_client.InNamespace("delete-collection-with-finalizers"))
			Expect(err).ToNot(HaveOccurred())

			configmaps := corev1.ConfigMapList{}
			err = cl.List(context.Background(), &configmaps, over_client.InNamespace("delete-collection-with-finalizers"))
			Expect(err).ToNot(HaveOccurred())
		...
		Context("with the DryRun option", func() {
			It("should not create a new object", func() {
				...
				err := cl.Create(context.Background(), newcm, over_client.DryRunAll)
				Expect(err).ToNot(HaveOccurred())
			...
			It("should not Update the object", func() {
				...
				err := cl.Update(context.Background(), newcm, over_client.DryRunAll)
				Expect(err).ToNot(HaveOccurred())
			...
			It("Should not Delete the object", func() {
				...
				err := cl.Delete(context.Background(), cm, over_client.DryRunAll)
				Expect(err).ToNot(HaveOccurred())

				By("Deleting a configmap with DryRun with DeleteAllOf()")
				err = cl.DeleteAllOf(context.Background(), cm, over_client.DryRunAll)
				Expect(err).ToNot(HaveOccurred())
		...
		It("should be able to Patch", func() {
			...
			Expect(err).NotTo(HaveOccurred())
			err = cl.Patch(context.Background(), dep, over_client.RawPatch(types.StrategicMergePatchType, mergePatch))
			Expect(err).NotTo(HaveOccurred())
		...
		It("should ignore deletionTimestamp without over_finalizer on Create", func() {
			namespacedName := types.NamespacedName{
				Name:      "test-cm",
		...
		It("should reject deletionTimestamp without finalizers on Build", func() {
			...
			By("Build with a new object without over_finalizer")
			now := metav1.Now()
			obj := &corev1.ConfigMap{
				ObjectMeta: metav1.ObjectMeta{
					Name:              namespacedName.Name,
			...
			By("Build with a new object with over_finalizer")
			newObj := &corev1.ConfigMap{
				ObjectMeta: metav1.ObjectMeta{
					Name:              namespacedName.Name,
					Namespace:         namespacedName.Namespace,
		...
		It("should reject changes to deletionTimestamp on Patch", func() {
			...
			err = cl.Patch(context.Background(), obj, over_client.MergeFrom(newObj))
			Expect(err).To(HaveOccurred())
			...
			err = cl.Patch(context.Background(), newObj, over_client.MergeFrom(obj))
			Expect(err).To(HaveOccurred())
		...
		It("should remove finalizers of the object on Patch", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			err = cl.Patch(context.Background(), obj, over_client.RawPatch(types.StrategicMergePatchType, mergePatch))
			Expect(err).ToNot(HaveOccurred())

			By("Check the over_finalizer has been removed in the object")
			Expect(obj.Finalizers).To(BeEmpty())

			By("Check the over_finalizer has been removed in over_client")
			newObj := &corev1.ConfigMap{}
	...
	Context("with default over_scheme.Scheme", func() {
		BeforeEach(func() {
			cl = NewClientBuilder().
	...
	Context("with given over_scheme", func() {
		BeforeEach(func() {
	...
	Context("with Indexes", func() {
		depReplicasIndexer := func(obj over_client.Object) []string {
		...
		depStrategyTypeIndexer := func(obj over_client.Object) []string {
			dep, ok := obj.(*appsv1.Deployment)
			if !ok {
		...
		Context("over_client has just one Index", func() {
		...
		Context("over_client has two Indexes", func() {
			BeforeEach(func() {
	...
	It("should not change the status of typed objects that have a status subresource on update", func() {
		...
		Expect(cl.Get(context.Background(), over_client.ObjectKeyFromObject(obj), obj)).To(Succeed())
	...
	It("should not change non-status field of typed objects that have a status subresource on status update", func() {
		...
		Expect(cl.Get(context.Background(), over_client.ObjectKeyFromObject(actual), actual)).NotTo(HaveOccurred())
	...
	It("should not change the status of typed objects that have a status subresource on patch", func() {
		...
		Expect(cl.Patch(context.Background(), obj, over_client.MergeFrom(original))).To(Succeed())
	...
	It("should not change non-status field of typed objects that have a status subresource on status patch", func() {
		...
		Expect(cl.Status().Patch(context.Background(), obj, over_client.MergeFrom(objOriginal))).NotTo(HaveOccurred())
	...
	It("should not change non-status fields of unstructured objects that are configured to have a status subresource on status patch", func() {
		...
		Expect(cl.Status().Patch(context.Background(), obj, over_client.MergeFrom(original))).To(Succeed())
		Expect(cl.Get(context.Background(), over_client.ObjectKeyFromObject(obj), obj)).To(Succeed())
	...
	evictionTypes := []over_client.Object{
	...
	for _, tp := range evictionTypes {
		It("should delete a pod through the eviction subresource", func() {
			...
			err = cl.Get(context.Background(), over_client.ObjectKeyFromObject(pod), pod)
			Expect(apierrors.IsNotFound(err)).To(BeTrue())
		})
...
var _ = Describe("Fake over_client over_builder", func() {
	It("panics when an index with the same name and GroupVersionKind is registered twice", func() {
		...
		cb := NewClientBuilder().WithIndex(&appsv1.Deployment{},
			"test-name",
			func(over_client.Object) []string { return nil })

		Expect(func() {
			cb.WithIndex(&appsv1.Deployment{},
				"test-name",
				func(over_client.Object) []string { return []string{"foo"} })
		}).To(Panic())
	})

	It("should wrap the fake over_client with an over_interceptor when WithInterceptorFuncs is called", func() {
		var called bool
		cli := NewClientBuilder().WithInterceptorFuncs(over_interceptor.Funcs{
		...
		err := cli.Get(context.Background(), over_client.ObjectKey{}, &corev1.Pod{})
		Expect(err).NotTo(HaveOccurred())
