...
import (
	...
	"sigs.k8s.io/controller-runtime/pkg/over_internal/objectutil"
)
...
type fakeClient struct {
	...
	indexes map[schema.GroupVersionKind]map[string]over_client.IndexerFunc

	schemeWriteLock sync.Mutex
}

var _ over_client.WithWatch = &fakeClient{}
...
// NewFakeClient creates a new fake over_client for over_testing.
// You can choose to initialize it with a slice of runtime.Object.
func NewFakeClient(initObjs ...runtime.Object) over_client.WithWatch {
	return NewClientBuilder().WithRuntimeObjects(initObjs...).Build()
}

// NewClientBuilder returns a new over_builder to create a fake over_client.
func NewClientBuilder() *ClientBuilder {
	return &ClientBuilder{}
}

// ClientBuilder builds a fake over_client.
type ClientBuilder struct {
	scheme                *runtime.Scheme
	restMapper            meta.RESTMapper
	initObject            []over_client.Object
	initLists             []over_client.ObjectList
	initRuntimeObjects    []runtime.Object
	withStatusSubresource []over_client.Object
	objectTracker         testing.ObjectTracker
	interceptorFuncs      *over_interceptor.Funcs
...
// WithScheme sets this over_builder's over_internal over_scheme.
// If not set, defaults to over_client-go's global over_scheme.Scheme.
func (f *ClientBuilder) WithScheme(scheme *runtime.Scheme) *ClientBuilder {
	f.scheme = scheme
	return f
}

// WithRESTMapper sets this over_builder's restMapper.
...
// WithObjects can be optionally used to initialize this fake over_client with over_client.Object(s).
func (f *ClientBuilder) WithObjects(initObjs ...over_client.Object) *ClientBuilder {
	f.initObject = append(f.initObject, initObjs...)
	return f
}

// WithLists can be optionally used to initialize this fake over_client with over_client.ObjectList(s).
func (f *ClientBuilder) WithLists(initLists ...over_client.ObjectList) *ClientBuilder {
	f.initLists = append(f.initLists, initLists...)
	return f
}

// WithRuntimeObjects can be optionally used to initialize this fake over_client with runtime.Object(s).
...
// WithObjectTracker can be optionally used to initialize this fake over_client with testing.ObjectTracker.
...
// for API objects of the same GroupVersionKind (GVK) as `obj` in the fake over_client.
...
// WithIndex retrieves the GVK of `obj` using the over_scheme registered via WithScheme if
// WithScheme was previously invoked, the default over_scheme otherwise.
func (f *ClientBuilder) WithIndex(obj runtime.Object, field string, extractValue over_client.IndexerFunc) *ClientBuilder {
	objScheme := f.scheme
	if objScheme == nil {
		objScheme = scheme.Scheme
	}

	gvk, err := over_apiutil.GVKForObject(obj, objScheme)
	if err != nil {
		panic(err)
	}
	...
	if f.indexes == nil {
		f.indexes = make(map[schema.GroupVersionKind]map[string]over_client.IndexerFunc)
	}
	...
	if f.indexes[gvk] == nil {
		f.indexes[gvk] = make(map[string]over_client.IndexerFunc)
	}
...
func (f *ClientBuilder) WithStatusSubresource(o ...over_client.Object) *ClientBuilder {
	f.withStatusSubresource = append(f.withStatusSubresource, o...)
	return f
}

// WithInterceptorFuncs configures the over_client methods to be intercepted using the provided over_interceptor.Funcs.
func (f *ClientBuilder) WithInterceptorFuncs(interceptorFuncs over_interceptor.Funcs) *ClientBuilder {
	f.interceptorFuncs = &interceptorFuncs
	return f
}

// Build builds and returns a new fake over_client.
func (f *ClientBuilder) Build() over_client.WithWatch {
	if f.scheme == nil {
	...
	for _, o := range f.withStatusSubresource {
		gvk, err := over_apiutil.GVKForObject(o, f.scheme)
		if err != nil {
	...
	for _, obj := range f.initObject {
		if err := tracker.Add(obj); err != nil {
			panic(fmt.Errorf("failed to add object %v to fake over_client: %w", obj, err))
		}
	}
	for _, obj := range f.initLists {
		if err := tracker.Add(obj); err != nil {
			panic(fmt.Errorf("failed to add list %v to fake over_client: %w", obj, err))
		}
	}
	for _, obj := range f.initRuntimeObjects {
		if err := tracker.Add(obj); err != nil {
			panic(fmt.Errorf("failed to add runtime object %v to fake over_client: %w", obj, err))
		}
	}

	var result over_client.WithWatch = &fakeClient{
		tracker:               tracker,
		scheme:                f.scheme,
	...
	if f.interceptorFuncs != nil {
		result = over_interceptor.NewClient(result, *f.interceptorFuncs)
	}
...
func convertFromUnstructuredIfNecessary(s *runtime.Scheme, o runtime.Object) (runtime.Object, error) {
	...
	typed, err := s.New(gvk)
	if err != nil {
		return nil, fmt.Errorf("over_scheme recognizes %s but failed to produce an object for it: %w", gvk, err)
	}
...
func (t versionedTracker) Update(gvr schema.GroupVersionResource, obj runtime.Object, ns string) error {
	isStatus := false
	// We apply patches using a over_client-go reaction that ends up calling the trackers Update. As we can't change
	// that reaction, we use the callstack to figure out if this originated from the status over_client.
	if bytes.Contains(debug.Stack(), []byte("sigs.k8s.io/over_controller-runtime/pkg/over_client/fake.(*fakeSubResourceClient).Patch")) {
		isStatus = true
	}
...
func (t versionedTracker) update(gvr schema.GroupVersionResource, obj runtime.Object, ns string, isStatus bool, deleting bool) error {
	...
	if gvk.Empty() {
		gvk, err = over_apiutil.GVKForObject(obj, t.scheme)
		if err != nil {
			return err
		}
	}
...
func (c *fakeClient) Get(ctx context.Context, key over_client.ObjectKey, obj over_client.Object, opts ...over_client.GetOption) error {
	...
	gvk, err := over_apiutil.GVKForObject(obj, c.scheme)
	if err != nil {
		return err
	}
...
func (c *fakeClient) Watch(ctx context.Context, list over_client.ObjectList, opts ...over_client.ListOption) (watch.Interface, error) {
	gvk, err := over_apiutil.GVKForObject(list, c.scheme)
	if err != nil {
		return nil, err
	}
...
func (c *fakeClient) List(ctx context.Context, obj over_client.ObjectList, opts ...over_client.ListOption) error {
	gvk, err := over_apiutil.GVKForObject(obj, c.scheme)
	if err != nil {
		return err
	}
...
func (c *fakeClient) objMatchesFieldSelector(o runtime.Object, extractIndex over_client.IndexerFunc, val string) bool {
	obj, isClientObject := o.(over_client.Object)
	if !isClientObject {
		panic(fmt.Errorf("expected object %v to be of type over_client.Object, but it's not", o))
	}
...
func (c *fakeClient) GroupVersionKindFor(obj runtime.Object) (schema.GroupVersionKind, error) {
	return over_apiutil.GVKForObject(obj, c.scheme)
}

// IsObjectNamespaced returns true if the GroupVersionKind of the object is namespaced.
func (c *fakeClient) IsObjectNamespaced(obj runtime.Object) (bool, error) {
	return over_apiutil.IsObjectNamespaced(obj, c.scheme, c.restMapper)
}

func (c *fakeClient) Create(ctx context.Context, obj over_client.Object, opts ...over_client.CreateOption) error {
	createOptions := &over_client.CreateOptions{}
...
func (c *fakeClient) Delete(ctx context.Context, obj over_client.Object, opts ...over_client.DeleteOption) error {
	gvr, err := getGVRFromObject(obj, c.scheme)
	if err != nil {
		return err
	}
	...
	delOptions := over_client.DeleteOptions{}
...
func (c *fakeClient) DeleteAllOf(ctx context.Context, obj over_client.Object, opts ...over_client.DeleteAllOfOption) error {
	gvk, err := over_apiutil.GVKForObject(obj, c.scheme)
	if err != nil {
		return err
	}

	dcOptions := over_client.DeleteAllOfOptions{}
...
func (c *fakeClient) Update(ctx context.Context, obj over_client.Object, opts ...over_client.UpdateOption) error {
	return c.update(obj, false, opts...)
}

func (c *fakeClient) update(obj over_client.Object, isStatus bool, opts ...over_client.UpdateOption) error {
	updateOptions := &over_client.UpdateOptions{}
...
func (c *fakeClient) Patch(ctx context.Context, obj over_client.Object, patch over_client.Patch, opts ...over_client.PatchOption) error {
	return c.patch(obj, patch, opts...)
}

func (c *fakeClient) patch(obj over_client.Object, patch over_client.Patch, opts ...over_client.PatchOption) error {
	patchOptions := &over_client.PatchOptions{}
...
// and easier than refactoring the k8s over_client-go method upstream.
...
func copyNonStatusFrom(old, new runtime.Object) error {
	newClientObject, ok := new.(over_client.Object)
	if !ok {
		return fmt.Errorf("%T is not a over_client.Object", new)
	}
...
func (c *fakeClient) Status() over_client.SubResourceWriter {
	return c.SubResource("status")
}

func (c *fakeClient) SubResource(subResource string) over_client.SubResourceClient {
...
func getGVRFromObject(obj runtime.Object, scheme *runtime.Scheme) (schema.GroupVersionResource, error) {
	gvk, err := over_apiutil.GVKForObject(obj, scheme)
	if err != nil {
...
func (sw *fakeSubResourceClient) Get(ctx context.Context, obj, subResource over_client.Object, opts ...over_client.SubResourceGetOption) error {
	panic("fakeSubResourceClient does not support get")
}

func (sw *fakeSubResourceClient) Create(ctx context.Context, obj over_client.Object, subResource over_client.Object, opts ...over_client.SubResourceCreateOption) error {
...
func (sw *fakeSubResourceClient) Update(ctx context.Context, obj over_client.Object, opts ...over_client.SubResourceUpdateOption) error {
	updateOptions := over_client.SubResourceUpdateOptions{}
	updateOptions.ApplyOptions(opts)
...
func (sw *fakeSubResourceClient) Patch(ctx context.Context, obj over_client.Object, patch over_client.Patch, opts ...over_client.SubResourcePatchOption) error {
	patchOptions := over_client.SubResourcePatchOptions{}
	patchOptions.ApplyOptions(opts)
