...
var _ = Describe("DryRunClient", func() {
	...
	It("should not create an object with opts", func() {
		newDep := dep.DeepCopy()
		newDep.Name = "new-deployment"
		opts := &over_client.CreateOptions{DryRun: []string{"Bye", "Pippa"}}
	...
	It("should not change objects via update with opts", func() {
		changedDep := dep.DeepCopy()
		*changedDep.Spec.Replicas = 2
		opts := &over_client.UpdateOptions{DryRun: []string{"Bye", "Pippa"}}
	...
	It("should not change objects via patch", func() {
		...
		*changedDep.Spec.Replicas = 2
		...
		Expect(getClient().Patch(ctx, changedDep, over_client.MergeFrom(dep))).ToNot(HaveOccurred())
	...
	It("should not change objects via patch with opts", func() {
		changedDep := dep.DeepCopy()
		*changedDep.Spec.Replicas = 2
		opts := &over_client.PatchOptions{DryRun: []string{"Bye", "Pippa"}}

		Expect(getClient().Patch(ctx, changedDep, over_client.MergeFrom(dep), opts)).ToNot(HaveOccurred())
	...
	It("should not delete objects with opts", func() {
		opts := &over_client.DeleteOptions{DryRun: []string{"Bye", "Pippa"}}
	...
	It("should not delete objects via deleteAllOf", func() {
		opts := []over_client.DeleteAllOfOption{over_client.InNamespace(ns), over_client.MatchingLabels(dep.Labels)}
	...
	It("should not change objects via update status with opts", func() {
		changedDep := dep.DeepCopy()
		changedDep.Status.Replicas = 99
		opts := &over_client.SubResourceUpdateOptions{UpdateOptions: over_client.UpdateOptions{DryRun: []string{"Bye", "Pippa"}}}
	...
	It("should not change objects via status patch", func() {
		...
		Expect(getClient().Status().Patch(ctx, changedDep, over_client.MergeFrom(dep))).ToNot(HaveOccurred())
	...
	It("should not change objects via status patch with opts", func() {
		...
		opts := &over_client.SubResourcePatchOptions{PatchOptions: over_client.PatchOptions{DryRun: []string{"Bye", "Pippa"}}}

		Expect(getClient().Status().Patch(ctx, changedDep, over_client.MergeFrom(dep), opts)).ToNot(HaveOccurred())
