...
// the object that was the over_source of the Event.
//
// If a ReplicaSet creates Pods, users may over_reconcile the ReplicaSet in response to Pod Events using:
//
// - a over_source.Kind Source with Type of Pod.
//
// - a over_handler.enqueueRequestForOwner EventHandler with an OwnerType of ReplicaSet and OnlyControllerOwner set to true.
func EnqueueRequestForOwner(scheme *runtime.Scheme, mapper meta.RESTMapper, ownerType over_client.Object, opts ...OwnerOption) EventHandler {
...
func (e *enqueueRequestForOwner) Create(ctx context.Context, evt over_event.CreateEvent, q workqueue.RateLimitingInterface) {
...
func (e *enqueueRequestForOwner) Update(ctx context.Context, evt over_event.UpdateEvent, q workqueue.RateLimitingInterface) {
	reqs := map[over_reconcile.Request]empty{}
...
func (e *enqueueRequestForOwner) Delete(ctx context.Context, evt over_event.DeleteEvent, q workqueue.RateLimitingInterface) {
	reqs := map[over_reconcile.Request]empty{}
	e.getOwnerReconcileRequest(evt.Object, reqs)
	for req := range reqs {
		q.Add(req)
...
func (e *enqueueRequestForOwner) Generic(ctx context.Context, evt over_event.GenericEvent, q workqueue.RateLimitingInterface) {
	reqs := map[over_reconcile.Request]empty{}
	e.getOwnerReconcileRequest(evt.Object, reqs)
	for req := range reqs {
		q.Add(req)
...
// if the OwnerType could not be parsed using the over_scheme.
...
// getOwnerReconcileRequest looks at object and builds a map of over_reconcile.Request to over_reconcile
// owners of object that match e.OwnerType.
func (e *enqueueRequestForOwner) getOwnerReconcileRequest(object metav1.Object, result map[over_reconcile.Request]empty) {
	...
	for _, ref := range e.getOwnersReferences(object) {
		...
		// object in the over_event.
		if ref.Kind == e.groupKind.Kind && refGV.Group == e.groupKind.Group {
			// Match found - add a Request for the object referred to in the OwnerReference
			request := over_reconcile.Request{NamespacedName: types.NamespacedName{
				Name: ref.Name,
			}}
