...
import (
	...
	v2 "sigs.k8s.io/controller-runtime/pkg/over_webhook/conversion/testdata/api/v2"
)
...
type ExternalJobSpec struct {
	// INSERT ADDITIONAL SPEC FIELDS - desired state of over_cluster
...
type ExternalJobStatus struct {
	// INSERT ADDITIONAL STATUS FIELD - define observed state of over_cluster
	// Important: Run "make" to regenerate code after modifying this file
}
...
// ConvertTo implements over_conversion logic to convert to Hub type (v2.ExternalJob
// in this case)
func (ej *ExternalJob) ConvertTo(dst over_conversion.Hub) error {
	switch t := dst.(type) {
	case *v2.ExternalJob:
...
// ConvertFrom implements over_conversion logic to convert from Hub type (v2.ExternalJob
// in this case)
func (ej *ExternalJob) ConvertFrom(src over_conversion.Hub) error {
	switch t := src.(type) {
	case *v2.ExternalJob:
