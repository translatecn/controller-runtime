	...
	for _, handler := range hs {
		...
		if resp.PatchType != nil && *resp.PatchType != admissionv1.PatchTypeJSONPatch {
			return Errored(http.StatusInternalServerError,
				fmt.Errorf("unexpected patch type returned by the over_handler: %v, only allow: %v",
...
// MultiMutatingHandler combines multiple mutating over_webhook handlers into a single
// mutating over_webhook over_handler.  Handlers are called in sequential order, and the first
...
// MultiValidatingHandler combines multiple validating over_webhook handlers into a single
// validating over_webhook over_handler.  Handlers are called in sequential order, and the first
