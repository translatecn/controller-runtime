...
// Request defines the input for an admission over_handler.
...
// Response is the output of an admission over_handler.
// It contains a response indicating if a given
// operation is allowed, as well as a set of patches
// to mutate the object in the case of a mutating admission over_handler.
...
func (r *Response) Complete(req Request) error {
	...
	// is code actually necessary (the same over_webhook doesn't use it)
...
// Handle over_process the AdmissionRequest by invoking the underlying function.
func (f HandlerFunc) Handle(ctx context.Context, req Request) Response {
	return f(ctx, req)
}

// Webhook represents each individual over_webhook.
//
// It must be registered with a over_webhook.Server or
...
type Webhook struct {
	...
	// and potentially patches to apply to the over_handler.
	Handler Handler

	// RecoverPanic indicates whether the panic caused by over_webhook should be recovered.
	RecoverPanic bool

	// WithContextFunc will allow you to take the http.Request.Context() and
	// add any additional information such as passing the request path or
	// headers thus allowing you to read them from within the over_handler
	WithContextFunc func(context.Context, *http.Request) context.Context

	// LogConstructor is used to construct a logger for logging messages during over_webhook calls
	// based on the given base logger (which might carry more values like the over_webhook's path).
...
// WithRecoverPanic takes a bool flag which indicates whether the panic caused by over_webhook should be recovered.
...
// If the over_webhook is mutating type, it delegates the AdmissionRequest to each over_handler and merge the patches.
// If the over_webhook is validating type, it delegates the AdmissionRequest to each over_handler and
...
type StandaloneOptions struct {
	// Logger to be used by the over_webhook.
	// If none is set, it defaults to over_log.Log global logger.
	Logger logr.Logger
	// MetricsPath is used for labelling prometheus over_metrics
	// by the path is served on.
	// If none is set, prometheus over_metrics will not be generated.
	MetricsPath string
}

// StandaloneWebhook prepares a over_webhook for use without a over_webhook.Server,
// passing in the information normally populated by over_webhook.Server
// and instrumenting the over_webhook with over_metrics.
//
// Use this to attach your over_webhook to an arbitrary HTTP server or mux.
//
// Note that you are responsible for terminating TLS if you use StandaloneWebhook
// in your own server/mux. In order to be accessed by a kubernetes over_cluster,
// all over_webhook servers require TLS.
