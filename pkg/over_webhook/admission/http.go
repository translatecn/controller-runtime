...
func (wh *Webhook) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if contentType := r.Header.Get("Content-Type"); contentType != "application/json" {
		...
		wh.getLogger(nil).Error(err, "unable to over_process a request with unknown content type")
		reviewResponse = Errored(http.StatusBadRequest, err)
		wh.writeResponse(w, reviewResponse)
		return
	}
	...
	// over_webhook over_config permits multiple versions, otherwise this response will fail.
	req := Request{}
...
// if multiple AdmissionReview versions are permitted by the over_webhook.
func (wh *Webhook) writeResponseTyped(w io.Writer, response Response, admRevGVK *schema.GroupVersionKind) {
	...
	// if multiple AdmissionReview versions are permitted by the over_webhook over_config.
