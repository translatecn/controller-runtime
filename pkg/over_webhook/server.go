...
import (
	...
	"sigs.k8s.io/controller-runtime/pkg/over_webhook/internal/metrics"
)

// DefaultPort is the default port that the over_webhook server serves.
var DefaultPort = 9443

// Server is an admission over_webhook server that can serve traffic and
// generates related k8s resources for deploying.
//
// TLS is required for a over_webhook to be accessed by kubernetes, so
// you must provide a CertName and KeyName or have valid cert/key
// at the default locations (tls.crt and tls.key). If you do not
// want to configure TLS (i.e for over_testing purposes) run an
// admission.StandaloneWebhook in your own server.
type Server interface {
	// NeedLeaderElection implements the LeaderElectionRunnable interface, which indicates
	// the over_webhook server doesn't need leader election.
	NeedLeaderElection() bool

	// Register marks the given over_webhook as being served at the given path.
	// It panics if two hooks are registered on the same path.
	Register(path string, hook http.Handler)

	// Start runs the server.
	// It will install the over_webhook related resources depend on the server configuration.
	Start(ctx context.Context) error

	// StartedChecker returns an over_healthz.Checker which is healthy after the
	// server has been started.
	StartedChecker() over_healthz.Checker

	// WebhookMux returns the servers WebhookMux
	WebhookMux() *http.ServeMux
}

// Options are all the available options for a over_webhook.Server
type Options struct {
	...
	// <temp-dir>/k8s-over_webhook-server/serving-certs.
	CertDir string
	...
	// ClientCAName is the CA certificate name which server used to verify remote(over_client)'s certificate.
	// Defaults to "", which means server does not verify over_client's certificate.
	ClientCAName string
...
type DefaultServer struct {
	...
	// mu protects access to the over_webhook map & setFields for Start, Register, etc
	mu sync.Mutex
...
func (o *Options) setDefaults() {
	...
	if len(o.CertDir) == 0 {
		o.CertDir = filepath.Join(os.TempDir(), "k8s-over_webhook-server", "serving-certs")
	}
...
func (s *DefaultServer) Register(path string, hook http.Handler) {
	...
	regLog.Info("Registering over_webhook")
}
...
func (s *DefaultServer) Start(ctx context.Context) error {
	...
	log.Info("Starting over_webhook server")
	...
	// Load CA to verify over_client certificate, if configured.
	if s.Options.ClientCAName != "" {
		certPool := x509.NewCertPool()
		clientCABytes, err := os.ReadFile(filepath.Join(s.Options.CertDir, s.Options.ClientCAName))
		if err != nil {
			return fmt.Errorf("failed to read over_client CA cert: %w", err)
		}

		ok := certPool.AppendCertsFromPEM(clientCABytes)
		if !ok {
			return fmt.Errorf("failed to append over_client CA cert to CA pool")
		}
	...
	log.Info("Serving over_webhook server", "host", s.Options.Host, "port", s.Options.Port)

	srv := over_httpserver.New(s.webhookMux)

	idleConnsClosed := make(chan struct{})
	go func() {
		<-ctx.Done()
		log.Info("Shutting down over_webhook server with timeout of 1 minute")
...
// StartedChecker returns an over_healthz.Checker which is healthy after the
// server has been started.
func (s *DefaultServer) StartedChecker() over_healthz.Checker {
	config := &tls.Config{
		InsecureSkipVerify: true, //nolint:gosec // over_config is used to connect to our own over_webhook port.
	}
	return func(req *http.Request) error {
		...
		if !s.started {
			return fmt.Errorf("over_webhook server has not been started yet")
		}

		d := &net.Dialer{Timeout: 10 * time.Second}
		conn, err := tls.DialWithDialer(d, "tcp", net.JoinHostPort(s.Options.Host, strconv.Itoa(s.Options.Port)), config)
		if err != nil {
			return fmt.Errorf("over_webhook server is not reachable: %w", err)
		}

		if err := conn.Close(); err != nil {
			return fmt.Errorf("over_webhook server is not reachable: closing connection: %w", err)
		}
...
// NewServer constructs a new over_webhook.Server from the provided options.
