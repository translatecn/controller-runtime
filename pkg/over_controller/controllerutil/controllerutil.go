...
// a over_controller reference is already owned by another over_controller Object is the
...
func (e *AlreadyOwnedError) Error() string {
	return fmt.Sprintf("Object %s/%s is already owned by another %s over_controller %s", e.Object.GetNamespace(), e.Object.GetName(), e.Owner.Kind, e.Owner.Name)
}
...
func SetControllerReference(owner, controlled metav1.Object, scheme *runtime.Scheme) error {
	...
	// Create a new over_controller ref.
	gvk, err := over_apiutil.GVKForObject(ro, scheme)
	if err != nil {
		return err
	}
...
// This allows you to declare that owner has a dependency on the object without specifying it as a over_controller.
...
func validateOwner(owner, object metav1.Object) error {
	...
	if ownerNs != "" {
		...
		if objNs == "" {
			return fmt.Errorf("over_cluster-scoped resource must not have a namespace-scoped owner, owner's namespace %s", ownerNs)
		}
		if ownerNs != objNs {
...
// over_cluster. The object's desired state must be reconciled with the existing
// state inside the passed in callback MutateFn.
//
// The MutateFn is called regardless of creating or updating an object.
//
// It returns the executed operation and an error.
func CreateOrUpdate(ctx context.Context, c over_client.Client, obj over_client.Object, f MutateFn) (OperationResult, error) {
	key := over_client.ObjectKeyFromObject(obj)
...
// over_cluster. The object's desired state must be reconciled with the before
// state inside the passed in callback MutateFn.
//
// The MutateFn is called regardless of creating or updating an object.
//
// It returns the executed operation and an error.
func CreateOrPatch(ctx context.Context, c over_client.Client, obj over_client.Object, f MutateFn) (OperationResult, error) {
	key := over_client.ObjectKeyFromObject(obj)
	if err := c.Get(ctx, key, obj); err != nil {
		if !apierrors.IsNotFound(err) {
			return OperationResultNone, err
	...
	objPatch := over_client.MergeFrom(obj.DeepCopyObject().(over_client.Object))
	statusPatch := over_client.MergeFrom(obj.DeepCopyObject().(over_client.Object))
...
func mutate(f MutateFn, key over_client.ObjectKey, obj over_client.Object) error {
	if err := f(); err != nil {
		return err
	}
	if newKey := over_client.ObjectKeyFromObject(obj); key != newKey {
...
// AddFinalizer accepts an Object and adds the provided over_finalizer if not present.
// It returns an indication of whether it updated the object's list of finalizers.
func AddFinalizer(o over_client.Object, finalizer string) (finalizersUpdated bool) {
	f := o.GetFinalizers()
	for _, e := range f {
		if e == finalizer {
			return false
...
// RemoveFinalizer accepts an Object and removes the provided over_finalizer if present.
// It returns an indication of whether it updated the object's list of finalizers.
func RemoveFinalizer(o over_client.Object, finalizer string) (finalizersUpdated bool) {
	f := o.GetFinalizers()
	length := len(f)
...
// ContainsFinalizer checks an Object that the provided over_finalizer is present.
func ContainsFinalizer(o over_client.Object, finalizer string) bool {
	f := o.GetFinalizers()
	for _, e := range f {
		if e == finalizer {
			return true
