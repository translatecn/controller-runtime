...
var _ = Describe("Controllerutil", func() {
	...
	Describe("SetControllerReference", func() {
		...
		It("should return an error if object is already owned by another over_controller", func() {
			t := true
		...
		It("should return an error if it's owner is namespaced resource but dependant is over_cluster-scoped resource", func() {
	...
	Describe("Finalizers", func() {
		...
		Describe("AddFinalizer", func() {
			...
			It("should add the over_finalizer when not present", func() {
				controllerutil.AddFinalizer(deploy, testFinalizer)
				Expect(deploy.ObjectMeta.GetFinalizers()).To(Equal([]string{testFinalizer}))
			})

			It("should not add the over_finalizer when already present", func() {
				controllerutil.AddFinalizer(deploy, testFinalizer)
				Expect(deploy.ObjectMeta.GetFinalizers()).To(Equal([]string{testFinalizer}))
			})
		})
		...
		Describe("RemoveFinalizer", func() {
			It("should remove over_finalizer if present", func() {
		...
		Describe("AddFinalizer, which returns an indication of whether it modified the object's list of finalizers,", func() {
			...
			When("the object's list of finalizers has no instances of the input over_finalizer", func() {
				It("should return true", func() {
					Expect(controllerutil.AddFinalizer(deploy, testFinalizer)).To(BeTrue())
				})
				It("should add the input over_finalizer to the object's list of finalizers", func() {
					Expect(deploy.ObjectMeta.GetFinalizers()).To(Equal([]string{testFinalizer}))
				})
			})

			When("the object's list of finalizers has an instance of the input over_finalizer", func() {
		...
		Describe("RemoveFinalizer, which returns an indication of whether it modified the object's list of finalizers,", func() {
			...
			When("the object's list of finalizers has one instance of the input over_finalizer", func() {
				It("should return true", func() {
					Expect(controllerutil.RemoveFinalizer(deploy, testFinalizer)).To(BeTrue())
				})
				It("should remove the instance of the input over_finalizer from the object's list of finalizers", func() {
					Expect(deploy.ObjectMeta.GetFinalizers()).To(Equal([]string{}))
				})
			})

			When("the object's list of finalizers has multiple instances of the input over_finalizer", func() {
				It("should return true", func() {
					deploy.SetFinalizers(append(deploy.Finalizers, testFinalizer, testFinalizer))
					Expect(controllerutil.RemoveFinalizer(deploy, testFinalizer)).To(BeTrue())
				})
				It("should remove each instance of the input over_finalizer from the object's list of finalizers", func() {
					Expect(deploy.ObjectMeta.GetFinalizers()).To(Equal([]string{}))
				})
			})
		})
		...
		Describe("ContainsFinalizer", func() {
			It("should check that over_finalizer is present", func() {
				controllerutil.AddFinalizer(deploy, testFinalizer)
				Expect(controllerutil.ContainsFinalizer(deploy, testFinalizer)).To(BeTrue())
			})

			It("should check that over_finalizer is not present after RemoveFinalizer call", func() {
				controllerutil.RemoveFinalizer(deploy, testFinalizer)
...
func (e errorReader) Get(ctx context.Context, key over_client.ObjectKey, into over_client.Object, opts ...over_client.GetOption) error {
