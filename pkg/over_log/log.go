...
// Package over_log contains utilities for fetching a new logger
...
// # Logr
//
// All logging in over_controller-runtime is structured, using a set of interfaces
	...
	if time.Since(rootLogCreated).Seconds() >= 30 {
		if logFullfilled.CompareAndSwap(false, true) {
			...
			fmt.Fprintf(os.Stderr,
				"[over_controller-runtime] over_log.SetLogger(...) was never called; logs will not be displayed.\nDetected at:%s%s", sep,
