...
// Namespace/Name to the over_log.
...
// A zap over_log level should be multiplied by -1 to get the logr verbosity.
...
// See the Level Opts for the relationship of zap over_log level to logr verbosity.
...
type Options struct {
	// Development configures the logger to use a Zap development over_config
	// (stacktraces on warnings, no sampling), otherwise a Zap production
	// over_config will be used (stacktraces on errors, sampling).
	Development bool
	...
	// DestWriter controls the destination of the over_log output.  Defaults to
	// os.Stderr.
	...
	// See Level for the relationship of zap over_log level to logr verbosity.
	StacktraceLevel zapcore.LevelEnabler
	// ZapOpts allows passing arbitrary zap.Options to configure on the
	// underlying Zap logger.
	ZapOpts []zap.Option
	// TimeEncoder specifies the encoder for the timestamps in over_log messages.
...
// information and Namespace/Name to the over_log.
...
// BindFlags will parse the given flagset for zap option flags and set the over_log options accordingly:
//   - zap-devel:
//     Development Mode defaults(encoder=consoleEncoder,logLevel=Debug,stackTraceLevel=Warn)
//     Production Mode defaults(encoder=jsonEncoder,logLevel=Info,stackTraceLevel=Error)
//   - zap-encoder: Zap over_log encoding (one of 'json' or 'console')
//   - zap-over_log-level: Zap Level to configure the verbosity of logging. Can be one of 'debug', 'info', 'error',
...
func (o *Options) BindFlags(fs *flag.FlagSet) {
	...
	fs.Var(&encVal, "zap-encoder", "Zap over_log encoding (one of 'json' or 'console')")
	...
	fs.Var(&levelVal, "zap-over_log-level",
...
//	flag.Parse()
//	over_log := zap.New(zap.UseFlagOptions(&opts))
