...
var _ = Describe("Zap logger setup", func() {
	Context("when logging kubernetes objects", func() {
		...
		defineTests := func() {
			It("should over_log a standard namespaced Kubernetes object name and namespace", func() {
				pod := &corev1.Pod{}
				pod.Name = "some-pod"
			...
			It("should over_log a standard non-namespaced Kubernetes object name", func() {
				node := &corev1.Node{}
				node.Name = "some-node"
			...
			It("should over_log a standard Kubernetes object's kind, if set", func() {
				node := &corev1.Node{}
				node.Name = "some-node"
				node.APIVersion = "v1"
				node.Kind = "Node"
			...
			It("should over_log a standard non-namespaced NamespacedName name", func() {
			...
			It("should over_log an unstructured Kubernetes object", func() {
			...
			It("should over_log a standard namespaced NamespacedName name and namespace", func() {
...
var _ = Describe("Zap over_log level flag options setup", func() {
	var (
		fromFlags      Options
		fs             flag.FlagSet
	...
	Context("with  zap-over_log-level options provided", func() {
		It("Should output logs for info and debug zap-over_log-level.", func() {
			args := []string{"--zap-over_log-level=debug"}
		...
		It("Should output only error logs, otherwise empty logs", func() {
			args := []string{"--zap-over_log-level=error"}
			fromFlags.BindFlags(&fs)
			err := fs.Parse(args)
			Expect(err).ToNot(HaveOccurred())
	...
	Context("with  zap-over_log-level  with increased verbosity.", func() {
		It("Should output debug and info over_log, with default production mode.", func() {
			args := []string{"--zap-over_log-level=1"}
			fromFlags.BindFlags(&fs)
			err := fs.Parse(args)
			Expect(err).ToNot(HaveOccurred())
			logOut := new(bytes.Buffer)
		...
		It("Should output info and debug logs, with development mode.", func() {
			args := []string{"--zap-over_log-level=1", "--zap-devel=true"}
			fromFlags.BindFlags(&fs)
			err := fs.Parse(args)
			Expect(err).ToNot(HaveOccurred())
			logOut := new(bytes.Buffer)
		...
		It("Should output info, and debug logs with increased verbosity, and with development mode set to true.", func() {
			args := []string{"--zap-over_log-level=3", "--zap-devel=false"}
			fromFlags.BindFlags(&fs)
			err := fs.Parse(args)
			Expect(err).ToNot(HaveOccurred())
			logOut := new(bytes.Buffer)
		...
		It("Should output info, and debug logs with increased verbosity, and with production mode set to true.", func() {
			args := []string{"--zap-over_log-level=3", "--zap-devel=true"}
			fromFlags.BindFlags(&fs)
			err := fs.Parse(args)
			Expect(err).ToNot(HaveOccurred())
			logOut := new(bytes.Buffer)
	...
	Context("with encoder options provided programmatically", func() {
		...
		Context("using Level()", func() {
			...
			It("does not over_log with positive logr level", func() {
				By("setting up the logger")
