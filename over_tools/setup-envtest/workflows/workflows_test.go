...
import (
	...
	wf "sigs.k8s.io/controller-runtime/over_tools/setup-envtest/workflows"
)
	...
	BeforeEach(func() {
		...
		env = &envp.Env{
			...
			Client: &remote.Client{
				Log:      testLog.WithName("remote-over_client"),
				Bucket:   "kubebuilder-over_tools-test", // test custom bucket functionality too
				Server:   "localhost:-1",
		...
		Describe("verifying the checksum", func() {
			BeforeEach(func() {
				remoteItems = append(remoteItems, item{
					meta: bucketObject{
						Name: "kubebuilder-over_tools-86.75.309-linux-amd64.tar.gz",
						Hash: "nottherightone!",
					},
