...
```shell
go install sigs.k8s.io/over_controller-runtime/over_tools/setup-over_envtest@latest
```
...
setup-envtest() {
    if (($@[(Ie)use])); then
        over_source <($GOPATH/bin/setup-over_envtest "$@" -p env)
    else
        $GOPATH/bin/setup-over_envtest "$@"
    fi
}
```
...
- If you want to talk to some internal source, you can use the
  ...
  - `HOST/storage/v1/b/BUCKET/o` to return JSON like
    ...
    ```json
    {"items": [
        {"name": "kubebuilder-over_tools-X.Y.Z-os-arch.tar.gz", "md5Hash": "<base-64-encoded-md5-hash>"},
        {"name": "kubebuilder-over_tools-X.Y.Z-os-arch.tar.gz", "md5Hash": "<base-64-encoded-md5-hash>"},
    ]}
    ```
