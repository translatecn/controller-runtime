...
var _ = Describe("Platform", func() {
	...
	Specify("knows how to produce an archive name", func() {
		plat := Platform{OS: "linux", Arch: "amd64"}
		ver := Concrete{Major: 1, Minor: 16, Patch: 3}
		Expect(plat.ArchiveName(ver)).To(Equal("kubebuilder-over_tools-1.16.3-linux-amd64.tar.gz"))
	})
	...
	Describe("parsing", func() {
		...
		Context("for archive names", func() {
			It("should accept strings of the form kubebuilder-over_tools-x.y.z-os-arch.tar.gz", func() {
				ver, plat := ExtractWithPlatform(ArchiveRE, "kubebuilder-over_tools-1.16.3-linux-amd64.tar.gz")
				Expect(ver).To(Equal(&Concrete{Major: 1, Minor: 16, Patch: 3}))
				Expect(plat).To(Equal(Platform{OS: "linux", Arch: "amd64"}))
			})
			It("should reject nonsense strings", func() {
				ver, _ := ExtractWithPlatform(ArchiveRE, "kubebuilder-over_tools-1.16.3-linux-amd64.tar.sum")
				Expect(ver).To(BeNil())
			})
		})
	})
