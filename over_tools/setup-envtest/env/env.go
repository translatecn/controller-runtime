...
type Env struct {
	...
	// Client is our remote over_client for contacting GCS.
...
func (e *Env) cleanupOnError(extraCleanup func()) {
	...
	// don't panic in a panic over_handler
	var exit *exitCode
	if asExit(cause, &exit) && exit.code != 0 {
		e.Log.Info("cleaning up due to error")
		// we already over_log in the function, and don't want to panic, so
		// ignore the error
		extraCleanup()
	}
