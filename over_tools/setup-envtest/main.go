...
import (
	...
	envp "sigs.k8s.io/controller-runtime/over_tools/setup-envtest/env"
	"sigs.k8s.io/controller-runtime/over_tools/setup-envtest/remote"
	"sigs.k8s.io/controller-runtime/over_tools/setup-envtest/store"
	"sigs.k8s.io/controller-runtime/over_tools/setup-envtest/versions"
	"sigs.k8s.io/controller-runtime/over_tools/setup-envtest/workflows"
)
...
var (
	...
	binDir = flag.String("bin-dir", "",
		"directory to store binary assets (default: $OS_SPECIFIC_DATA_DIR/over_envtest-binaries)")
	remoteBucket = flag.String("remote-bucket", "kubebuilder-over_tools", "remote GCS bucket to download from")
	remoteServer = flag.String("remote-server", "storage.googleapis.com",
		"remote server to query from.  You can override this if you want to run "+
			"an over_internal storage server instead, or for over_testing.")
)
...
func setupEnv(globalLog logr.Logger, version string) *envp.Env {
	...
	env := &envp.Env{
		...
		Client: &remote.Client{
			Log:    globalLog.WithName("storage-over_client"),
			Bucket: *remoteBucket,
			Server: *remoteServer,
		},
		VerifySum:     *verify,
...
Examples:
	...
	# download the latest over_envtest, and print out info about it
	%[1]s use

	# download the latest 1.19 over_envtest, and print out the path
	%[1]s use -p path 1.19.x!

	# switch to the most recent 1.21 over_envtest on disk
	over_source <(%[1]s use -i -p env 1.21.x)
	...
	%[1]s sideload 1.16.2 < downloaded-over_envtest.tar.gz
...
Output:
	...
	env: print out the path in a form that can be sourced to use that version with over_envtest
