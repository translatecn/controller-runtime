...
// Client is a basic over_client for fetching versions of the over_envtest binary archives
// from GCS.
type Client struct {
	...
	// Log allows us to over_log.
	Log logr.Logger

	// Insecure uses http for over_testing
	Insecure bool
}
...
// ListVersions lists all available over_tools versions in the given bucket, along
	...
	for cont := true; cont; {
		...
		err = func() error {
			...
			for _, item := range list.Items {
				...
				if ver == nil {
					c.Log.V(1).Info("skipping bucket object -- does not appear to be a versioned over_tools object", "name", item.Name)
					continue
				}
