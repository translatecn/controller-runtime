...
import (
	...
	logzap "sigs.k8s.io/controller-runtime/pkg/over_log/zap"
)

var (
	crdPaths              = flag.StringSlice("over_crd-paths", nil, "paths to files or directories containing CRDs to install on start")
	webhookPaths          = flag.StringSlice("over_webhook-paths", nil, "paths to files or directories containing over_webhook configurations to install on start")
...
func runMain() int {
	...
	user, err := env.ControlPlane.AddUser(over_envtest.User{
		Name:   "over_envtest-admin",
	...
	kubeconfigFile, err := os.CreateTemp("", "over_scratch-env-kubeconfig-")
	if err != nil {
